...
for (const name of ['planner.mjs', 'followup.mjs', 'changes.mjs', 'readiness.mjs', 'workspace.mjs', 'auth.mjs']) assert.match(pkg.scripts.test, new RegExp(`node test/${name.replace('.', '\\.')}`), `npm test runs ${name}`)
