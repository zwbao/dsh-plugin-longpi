...
assert.equal(mod.PRODUCT_VERSION, '0.5.1')
...
for (const name of ['planner.mjs', 'followup.mjs', 'changes.mjs', 'readiness.mjs', 'workspace.mjs', 'auth.mjs', 'connection.mjs', 'indicators.mjs']) assert.match(pkg.scripts.test, new RegExp(`node test/${name.replace('.', '\\.')}`), `npm test runs ${name}`)
assert.equal(require('../package-lock.json').version, '0.5.1')
