	...
	factory: (require) => {
		...
		*/
		...
		function PlanDraftCard(props) {
			...
			if (!data.draft) {
				...
				return h$16("div", { className: "lp-card lp-draft" }, h$16("div", { className: "lp-kicker" }, "方案草稿"), h$16("h3", { className: "lp-h3 lp-draft-title" }, "现在还起草不了方案"), h$16("p", { className: "lp-muted" }, why[0] || "你的记录里还没有能对上研究证据的指标。"), ...why.slice(1).map((text) => h$16("p", {
					key: text,
					className: "lp-fine"
		...
		*/
		...
		function HomeRow(props) {
			...
			usePendingVersion();
			const note = bridgeNote ?? (!ready && hasPendingPrompt() ? "先在下方选择一个工作区，选好后会自动放进输入框" : null);
		...
		*/
		...
		function Spark(props) {
			...
			}), dated ? h$6("div", { className: "lp-caption" }, `浅色带：以 ${row.compare.from_date} 的 ${fmtAuto(base)} 为基线的正常波动`) : null);
		}
		...
		function ChangeRow(props) {
			...
			return h$6("li", { className: "lp-change" }, h$6("div", { className: "lp-change-main" }, h$6("div", { className: "lp-strong" }, row.label_zh), h$6("p", { className: "lp-change-text" }, withoutLabel(row)), row.advice_zh ? h$6("p", { className: `lp-change-advice lp-change-${tone}` }, h$6(Icon, {
				name: row.ask_doctor ? "warn" : "check",
				size: 14
			}), h$6("span", null, row.advice_zh)) : null, row.caveat_zh ? h$6("p", { className: "lp-caption lp-change-caveat" }, row.caveat_zh) : null, row.source.title ? h$6("p", { className: "lp-caption lp-change-source" }, "波动数据来源：", row.source.url ? h$6("a", {
				href: row.source.url,
				target: "_blank",
				rel: "noopener noreferrer"
			}, row.source.title) : row.source.title, row.verified ? null : "（引用尚未逐字核对）") : null), h$6(Spark, { row }));
		}
		function ChangesCard(props) {
			...
			return h$6("section", {
				...
				className: "lp-label",
				id: "lp-changes-title"
			}, "记录里的明显变化", h$6("span", { className: "lp-optional" }, `${rows.length} 项`)), h$6("ul", { className: "lp-change-list" }, ...rows.map((row) => h$6(ChangeRow, {
				key: row.key,
				row
			}))), props.journey.changes_note_zh ? h$6("p", { className: "lp-fine" }, props.journey.changes_note_zh) : null);
		}
		//#endregion
