window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function VerdictChip(props) {
			...
			return h$16("span", { className: `lp-chip-v ${style.className}` }, h$16(Icon, {
				name: style.icon,
				size: 14
			}), label);
		}
		...
		function Tooltip(props) {
			...
			return h$15("div", {
				...
				role: "status"
			}, h$15("div", { className: "lp-tip-title" }, tip.title), ...tip.rows.map((row, index) => h$15("div", {
				className: "lp-tip-row",
				key: index
			}, h$15("span", { className: "lp-tip-value" }, row.value), h$15("span", { className: "lp-tip-label" }, row.label))));
		}
		function LineChart(props) {
			...
			if (points.length === 0) return h$15("div", {
				ref,
			...
			return h$15("div", {
			...
			}, ...yTicks.map((value) => h$15("g", { key: `g${value}` }, h$15("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(value),
				y2: y(value),
				...
				className: "lp-axis",
				textAnchor: "end"
			}, fmt(value, value % 1 === 0 ? 0 : digits)))), props.band ? h$15("rect", {
				x: bandFrom,
				...
				className: "lp-band",
				rx: 3
			}) : null, props.reference ? h$15("g", null, h$15("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(props.reference.value),
				y2: y(props.reference.value),
				className: "lp-ref"
			}), props.compact ? null : h$15("text", {
				x: width - pad.right + 4,
				y: y(props.reference.value) + 3,
				className: "lp-axis"
			}, props.reference.label)) : null, props.goal != null ? h$15("g", null, h$15("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(props.goal),
				y2: y(props.goal),
			...
			}, `目标 ${fmt(props.goal, digits)}`)) : null, h$15("path", {
				d: area,
				className: "lp-area"
			}), h$15("path", {
				d: path,
				className: "lp-line"
			}), focus != null ? h$15("line", {
				x1: x(days[focus]),
				x2: x(days[focus]),
				y1: pad.top,
				y2: height - pad.bottom,
				className: "lp-cross"
			}) : null, ...points.map((point, index) => points.length > 24 && index !== points.length - 1 && index !== focus ? null : h$15("circle", {
				key: `p${index}`,
				cx: x(days[index]),
				cy: y(point.value),
				r: focus === index ? 5.5 : 4,
				...
				y: y(last.value) + 4,
				className: "lp-end"
			}, `${fmt(last.value, digits)}`), props.compact ? null : h$15("text", {
				x: pad.left,
				y: height - 6,
				className: "lp-axis"
			}, monthLabel(points[0]?.date ?? "")), props.compact || points.length < 2 ? null : h$15("text", {
				x: width - pad.right,
				y: height - 6,
				className: "lp-axis",
				textAnchor: "end"
			}, monthLabel(last.date))), h$15(Tooltip, {
				tip,
				width
			}));
		}
		function Timeline(props) {
			...
			return h$15("div", { className: "lp-timeline" }, h$15("div", {
				ref,
				className: "lp-chart",
				style: { height }
			}, h$15("svg", {
			...
			}, ...months.filter((_, index) => index % step === 0).map((iso) => h$15("g", { key: iso }, h$15("line", {
				x1: x(dayNumber(iso)),
				x2: x(dayNumber(iso)),
				y1: 20,
				y2: height - 6,
				...
				y: 12,
				className: "lp-axis"
			}, monthLabel(iso)))), ...props.checkups.map((iso) => h$15("g", { key: `c${iso}` }, h$15("line", {
				x1: x(dayNumber(iso)),
				x2: x(dayNumber(iso)),
				y1: 22,
				y2: height - 6,
				...
				}, item.title.length > 12 ? `${item.title.slice(0, 11)}…` : item.title), h$15("rect", {
					x: labelWidth,
					y: y0 - 12,
				...
				tip,
				width
			})), h$15("div", { className: "lp-legend-inline" }, h$15("span", { className: "lp-key-bar" }), "执行中", h$15("span", { className: "lp-key-dot" }), "体检日"));
		}
		function AdherenceStrip(props) {
			...
			}, ...props.calendar.map((day, index) => h$15("rect", {
				key: day.date,
		...
		function LeverBars(props) {
			...
			return h$15("div", {
				...
				return h$15("g", { key: item.label }, h$15("text", {
					x: 0,
					y: y0 + 14,
					className: "lp-row-label"
				}, item.label, item.detail ? h$15("tspan", {
					dx: 8,
					className: "lp-axis"
				}, item.detail) : null), h$15("path", {
		...
		function TableTwin(props) {
			return h$15("details", { className: "lp-twin" }, h$15("summary", null, "表格"), h$15("table", null, h$15("caption", null, props.caption), h$15("thead", null, h$15("tr", null, ...props.head.map((cell) => h$15("th", {
				key: cell,
				scope: "col"
			}, cell)))), h$15("tbody", null, ...props.rows.map((row, index) => h$15("tr", { key: index }, ...row.map((cell, column) => h$15("td", { key: column }, cell)))))));
		}
		//#endregion
		...
		function selfOf(raw) {
			return {
				latest: objects(raw.latest).filter((row) => SELF_KEYS.includes(row.key) && num(row.value) != null).map((row) => ({
					key: row.key,
					label_zh: str(row.label_zh),
					value: num(row.value),
					unit: str(row.unit),
		...
		const SHAPE = { journey: normalizeJourney };
		...
		*/
		...
		function Section(props) {
			...
			}, h$14("div", { className: "lp-section-head" }, h$14("div", { className: "lp-section-titles" }, props.kicker ? h$14("div", { className: "lp-kicker" }, props.kicker) : null, h$14("h2", {
				className: "lp-h2",
				id: headingId
		...
		function Btn(props) {
			return h$14(_deepseek_ai_dsh_client_ui_primitives.Button, {
				variant: "primary",
				size: "md",
				...props
			});
		...
		function Segmented(props) {
			...
			return h$14("div", { className: "lp-seg-wrap" }, h$14("span", {
				id: labelId,
				className: props.hideLabel ? "lp-sr" : "lp-field-label"
			}, props.label), h$14("div", {
				className: "lp-seg",
				role: "radiogroup",
				"aria-labelledby": labelId
			}, ...props.options.map((option) => h$14("label", {
				key: option.value,
		...
		function SmartDock(props) {
			...
			const place = react.default.useCallback((text) => {
				...
				copyText(text).then((copied) => setMessage(copied ? "没能放进输入框，已复制，粘贴即可" : "没能放进输入框，请手动输入"));
			}, []);
			react.default.useEffect(() => {
			...
			react.default.useEffect(() => {
				...
				const timer = window.setTimeout(() => setMessage(null), 4e3);
				return () => window.clearTimeout(timer);
			}, [message]);
			const suggestions = journey?.suggestions ?? [];
			if (suggestions.length === 0) return null;
			return h$13("div", {
				className: "lp lp-dock",
				role: "group",
				"aria-label": "LongPi 建议的问题"
			}, h$13("span", { className: "lp-dock-mark" }, h$13(Mark, { size: 14 }), "LongPi"), ...suggestions.slice(0, 3).map((row) => h$13("button", {
				key: row.id,
				type: "button",
				className: "lp-dock-pill",
				title: "放进输入框",
				onClick: () => place(row.text_zh)
			}, row.text_zh)), message ? h$13("span", {
				className: "lp-dock-note",
				role: "status"
			}, message) : null);
		}
		//#endregion
		...
		function metrics(journey) {
			...
			const body = bioage.status === "ok" ? {
				key: "bioage",
				label: "身体年龄",
				value: fmt(bioage.phenoage),
				unit: "岁",
				sub: versusAge(bioage.advance) || "模型估计",
				good: bioage.advance != null && bioage.advance < -.5
			} : {
				key: "bioage",
				label: "身体年龄",
				value: "—",
			...
			const heart = risk.status === "ok" ? {
				key: "risk",
				label: "心血管 10 年风险",
				value: riskText(risk.risk_pct),
				unit: "%",
				sub: risk.category_zh || "模型估计"
			} : {
				key: "risk",
				label: "心血管 10 年风险",
				value: "—",
			...
			if (!plan.exists) third = {
				...
				third = {
					key: "today",
					label: "今日打卡",
					value: `${done}/${plan.checkin_items.length}`,
					unit: "",
					sub: done === plan.checkin_items.length ? "今天都完成了" : `还有 ${plan.checkin_items.length - done} 项`,
					good: done === plan.checkin_items.length
				};
			} else third = {
				key: "adherence",
				label: "方案执行率",
				value: plan.adherence_pct == null ? "—" : String(plan.adherence_pct),
				unit: plan.adherence_pct == null ? "" : "%",
				sub: plan.streak > 1 ? `连续 ${plan.streak} 天` : "近 12 周"
			};
		...
		*/
		...
		function Shell(props) {
			...
			return h$12("div", {
				ref,
				className: `lp lp-home ${props.className ?? ""}`.trim(),
				role: "region",
				"aria-label": props.label
			}, props.children);
		}
		function Prompt(props) {
			return h$12(Shell, { label: `LongPi：${props.title}` }, h$12("div", { className: "lp-home-row" }, h$12("span", { className: "lp-home-mark" }, h$12(Mark, { size: 18 })), h$12("div", { className: "lp-home-text" }, h$12("div", { className: "lp-home-title" }, props.title), h$12("div", { className: "lp-home-detail" }, props.detail), props.extra ?? null), h$12(Btn, {
				size: "sm",
		...
		function HomeCard(props) {
			...
			if (!journey) {
				...
				return h$12(Shell, {
					className: "lp-home-loading",
					label: "LongPi 正在读取"
				}, h$12("div", { className: "lp-home-row" }, h$12("span", { className: "lp-home-mark lp-skeleton-mark" }), h$12("div", { className: "lp-home-text" }, h$12("div", {
			...
			if (stage === "consent" || stage === "profile") return h$12(Prompt, {
				title: "开始建档",
				detail: "约 2 分钟，解锁身体年龄和心血管风险",
				action: "开始",
				onAction: open
			});
			if (stage === "records") return h$12(Prompt, {
				title: "连接体检记录",
				detail: journey.records.status === "error" ? `记录读取失败：${journey.records.error || "没有返回原因"}` : "在 Mirobody 上传体检报告或连接手环，LongPi 就能算出第一个结果",
				action: "查看",
				onAction: open
			});
			if (stage === "first_result" && journey.addons.length === 0) return h$12(Prompt, {
				...
				action: journey.next.action === "profile" ? "去填写" : "查看",
				onAction: open
			});
			if (stage === "first_result") {
				...
				return h$12(Prompt, {
					...
					action: "查看加测清单",
					onAction: open,
					extra: top.length > 0 ? h$12("div", { className: "lp-home-chips" }, ...top.map((row) => h$12("span", {
						key: row.item_zh,
						className: "lp-need"
					}, row.self_measurable ? h$12(Icon, {
						name: "ruler",
						size: 12
					}) : null, row.item_zh))) : null
				});
			}
			return h$12(Shell, { label: "LongPi 今日概览" }, h$12("div", { className: "lp-home-head" }, h$12("span", { className: "lp-home-brand" }, h$12(Mark, { size: 16 }), "LongPi"), h$12("span", { className: "lp-tag" }, "模型估计"), h$12("button", {
				type: "button",
				className: "lp-home-link",
				onClick: open
			}, "打开健康页", h$12(Icon, {
				name: "arrow",
				size: 14
			}))), h$12("div", { className: "lp-home-metrics" }, ...metrics(journey).map((row) => h$12("div", {
				key: row.key,
				className: "lp-home-metric"
			}, h$12("div", { className: "lp-caption" }, row.label), h$12("div", { className: "lp-home-value" }, row.value, row.unit ? h$12("span", { className: "lp-home-unit" }, row.unit) : null), h$12("div", { className: `lp-home-sub ${row.good ? "lp-good-ink" : ""}` }, row.sub)))));
		}
		//#endregion
		...
		function BodyAgeCard(props) {
			...
			return h$10("div", { className: `lp-card lp-result ${real ? "lp-result-win" : ""}` }, h$10(CardHead, { label: "身体年龄 · 表型年龄" }), h$10("div", { className: "lp-result-figure" }, h$10("span", { className: "lp-bignum" }, fmt(phenoage)), h$10("span", { className: "lp-bignum-unit" }, "岁"), band != null ? h$10("span", { className: "lp-band-note" }, `正常波动 ±${fmt(band)} 岁`) : null), h$10("div", { className: "lp-result-sub" }, versus ? h$10("span", { className: `lp-pill ${advance != null && advance < -.5 ? "lp-pill-good" : ""}` }, versus) : null, date ? h$10("span", { className: "lp-caption" }, `${chineseDate(date)}体检 · 共 ${count} 次完整血检`) : null), points.length > 1 ? h$10(LineChart, {
		...
		function RecordsGuide(props) {
			...
			if (records.status === "ok") return h$9("div", { className: "lp-step-body" }, h$9("div", { className: "lp-connected" }, h$9("span", {
				...
				name: "check",
				size: 16
			})), h$9("div", null, h$9("div", { className: "lp-strong" }, `已连接 · ${records.indicator_count} 项指标 · ${records.full_checkups} 次完整体检`), h$9("div", { className: "lp-caption" }, records.latest_checkup ? `最近一次完整体检：${chineseDate(records.latest_checkup)}` : "还没有九项血检测齐的一次体检"))), h$9(CanCompute, { journey: props.journey }), h$9("div", { className: "lp-form-actions" }, recheckButton));
		...
		function FirstResult(props) {
			...
			return h$9("div", { className: "lp-step-body" }, !props.showResults && blocked === 0 ? h$9(CanCompute, { journey: props.journey }) : null, !props.showResults ? null : h$9("div", { className: "lp-first" }, h$9("div", { className: "lp-first-cell" }, h$9("div", { className: "lp-caption" }, "身体年龄 · 模型估计"), bioage.status === "ok" ? h$9("div", null, h$9("div", { className: "lp-first-figure" }, fmt(bioage.phenoage), h$9("span", { className: "lp-bignum-unit" }, "岁")), h$9("div", { className: "lp-caption" }, [versusAge(bioage.advance), bioage.band_years != null ? `正常波动 ±${fmt(bioage.band_years)} 岁` : ""].filter(Boolean).join(" · "))) : h$9("p", { className: "lp-blocker" }, bioage.blocker_zh)), h$9("div", { className: "lp-first-cell" }, h$9("div", { className: "lp-caption" }, "10 年心血管病风险 · 模型估计"), risk.status === "ok" ? h$9("div", null, h$9("div", { className: "lp-first-figure" }, riskText(risk.risk_pct), h$9("span", { className: "lp-bignum-unit" }, "%")), h$9("div", { className: "lp-caption" }, [risk.category_zh, "China-PAR"].filter(Boolean).join(" · "))) : h$9("p", { className: "lp-blocker" }, risk.blocker_zh))), blocked > 0 ? h$9("div", { className: "lp-first-addons" }, h$9("div", { className: "lp-subhead" }, `还差 ${blocked} 项检查`, h$9("span", { className: "lp-optional" }, "下次体检加测，或现在自己量")), h$9(AddonList, {
				journey: props.journey,
				onNotice: props.onNotice,
		...
		function Onboarding(props) {
			...
			return h$7(OnboardingModal, { title: TITLES[step] ?? TITLES[0] }, h$7("div", {
				...
				showResults: true
			}), notice, h$7("p", { className: "lp-fine" }, journey.boundary_zh), h$7("div", { className: "lp-modal-actions" }, h$7(Btn, {
				variant: "outline",
				onClick: finish
			}, "完成"), h$7(Btn, {
				"data-modal-autofocus": true,
		...
		function Goals(props) {
			...
			return h$6(Section, {
				...
				title: "如果达到目标",
				kicker: "模型估计"
			}, h$6("div", { className: "lp-grid-goals" }, pheno ? h$6("div", { className: "lp-card lp-model" }, h$6("div", { className: "lp-label" }, "表型年龄"), pheno.goal ? h$6("div", { className: "lp-model-figures" }, h$6("div", null, h$6("div", { className: "lp-caption" }, "现在"), h$6("div", { className: "lp-tile-figure" }, `${fmt(pheno.now?.phenoage)} 岁`)), h$6(Icon, {
				name: "arrow",
				size: 18,
				className: "lp-muted-ink"
			}), h$6("div", null, h$6("div", { className: "lp-caption" }, "达到方案目标"), h$6("div", { className: "lp-tile-figure lp-good-ink" }, `${fmt(pheno.goal.phenoage)} 岁`)), h$6("span", { className: "lp-pill lp-pill-good" }, `${fmt(pheno.goal.phenoage_delta)} 岁`)) : h$6("p", { className: "lp-muted" }, pheno.note_zh ?? ""), leverRows.length > 0 ? h$6("div", null, h$6("div", { className: "lp-subhead" }, "每个目标单独的贡献"), h$6(LeverBars, { rows: leverRows })) : sensitivityRows.length > 0 ? h$6("div", null, h$6("div", { className: "lp-subhead" }, "对你的表型年龄影响最大的指标"), h$6(LeverBars, { rows: sensitivityRows })) : null, pheno.goal && pheno.now?.mortality_10y_pct != null && pheno.goal.mortality_10y_pct != null ? h$6("p", { className: "lp-fine" }, `同一模型的 10 年死亡风险：${pheno.now.mortality_10y_pct.toFixed(1)}% → ${pheno.goal.mortality_10y_pct.toFixed(1)}%。`) : null, h$6("p", { className: "lp-fine" }, `${pheno.measured_on ? `按 ${pheno.measured_on} 的血检计算。` : ""}${pheno.boundary_zh ?? ""}`)) : null, risk ? h$6("div", { className: "lp-card lp-model" }, h$6("div", { className: "lp-label" }, "10 年心血管病风险 · China-PAR"), risk.status === "unavailable" ? h$6("div", null, h$6("div", { className: "lp-tile-figure lp-muted-ink" }, (risk.missing ?? []).length > 0 ? "还差几项" : "暂不显示"), h$6("p", { className: "lp-muted" }, risk.note_zh ?? "")) : h$6("div", null, h$6("div", { className: "lp-model-figures" }, h$6("div", null, h$6("div", { className: "lp-caption" }, "现在"), h$6("div", { className: "lp-tile-figure" }, risk.now?.risk_pct == null ? "—" : `${risk.now.risk_pct.toFixed(1)}%`), risk.category_zh?.now ? h$6("span", { className: "lp-pill" }, risk.category_zh.now) : null), risk.goal ? h$6(Icon, {
				name: "arrow",
				size: 18,
				className: "lp-muted-ink"
			}) : null, risk.goal ? h$6("div", null, h$6("div", { className: "lp-caption" }, "达到方案目标"), h$6("div", { className: "lp-tile-figure lp-good-ink" }, risk.goal.risk_pct == null ? "—" : `${risk.goal.risk_pct.toFixed(1)}%`), risk.category_zh?.goal ? h$6("span", { className: "lp-pill lp-pill-good" }, risk.category_zh.goal) : null) : null), (risk.levers ?? []).length > 0 ? h$6("div", null, h$6("div", { className: "lp-subhead" }, "每个目标单独的贡献"), h$6(LeverBars, { rows: (risk.levers ?? []).map((row) => ({
				label: row.label,
				detail: `${row.from} → ${row.to}`,
				value: row.years,
				unit: "个百分点"
			})) })) : h$6("p", { className: "lp-muted" }, risk.note_zh ?? "")), h$6("p", { className: "lp-fine" }, risk.boundary_zh ?? "")) : null, h$6("div", { className: "lp-card lp-model lp-model-note" }, h$6("div", { className: "lp-label" }, "关于“能多活几年”"), h$6("p", { className: "lp-muted" }, "没有经过验证的模型能对个人给出“多活几年”。这里只给有依据的模型估计：表型年龄、同一模型的 10 年死亡风险，以及中国人群的 10 年心血管病风险。"), h$6("p", { className: "lp-fine" }, "试验里的平均效应都不大：热量限制使衰老速度慢约 2–3%，鱼油三年约慢 3 个月。能坚持的小改变，比追逐一个数字更重要。"))));
		}
		const STEP_ICON = {
			retest: "calendar",
		...
		function NextSteps(props) {
			...
			return h$6(Section, {
				...
				title: "下一步",
				kicker: "按优先级"
			}, h$6("ol", { className: "lp-card lp-steps" }, ...rows.map((row, index) => h$6("li", {
				key: index,
				className: `lp-step lp-step-${row.kind}`
			}, h$6("span", { className: "lp-step-icon" }, h$6(Icon, {
				name: STEP_ICON[row.kind] ?? "info",
				size: 15
			})), h$6("span", null, row.text_zh)))), h$6("p", { className: "lp-fine" }, "这里不会建议开始、停止或调整任何药物和补剂，也不给剂量。"));
		}
		//#endregion
		...
		function RunReady(props) {
			...
			return h$5("div", { className: "lp-card" }, h$5("div", { className: "lp-label" }, "你的记录现在就能算"), ready.length === 0 ? h$5("p", { className: "lp-muted" }, "还没有能直接计算的方法。") : h$5("ul", { className: "lp-rows" }, ...ready.slice(0, 8).map((row) => h$5("li", {
				key: row.name,
				className: "lp-row lp-row-stack"
			}, h$5("span", { className: "lp-strong" }, row.blurb || row.name), h$5("span", { className: "lp-caption" }, row.domain || row.name)))), ready.length > 8 ? h$5("p", { className: "lp-caption" }, `另有 ${ready.length - 8} 项`) : null, ready.length > 0 ? h$5("div", { className: "lp-form-actions" }, h$5(Btn, {
				size: "sm",
				disabled: running,
				onClick: () => {
					run();
				...
				name: "play",
				size: 13
			}), running ? "正在计算…" : `一键计算 ${ready.length} 项`)) : null, results ? h$5("ul", { className: "lp-rows lp-run" }, ...results.map((row) => h$5("li", {
				key: row.skill,
				className: "lp-row lp-row-stack"
			}, h$5("span", null, h$5("span", { className: row.ok ? "lp-good-ink" : "lp-muted-ink" }, row.ok ? "✓ " : "· "), row.skill), h$5("span", { className: "lp-caption" }, row.excerpt.split("\n")[0] ?? "")))) : null);
		}
		function Search(props) {
			...
			return h$5("div", { className: "lp-card" }, h$5("label", {
				className: "lp-label",
				htmlFor: "lp-method-q"
			}, "找方法"), h$5("form", {
				className: "lp-search",
			...
			}, busy ? "匹配中" : "匹配")), error ? h$5("p", { className: "lp-form-error" }, error) : null, note ? h$5("p", { className: "lp-fine" }, note) : null, shown.length > 0 ? h$5("ul", { className: "lp-rows" }, ...shown.slice(0, 6).map((item) => h$5("li", {
				key: item.name,
				className: "lp-row lp-row-stack"
			}, h$5("span", { className: "lp-strong" }, item.name), h$5("span", { className: "lp-caption" }, (item.why ?? []).join("；") || item.blurb || "")))) : null);
		}
		function MethodsSection(props) {
			const board = props.board;
			if (!board) return h$5(Section, {
				id: "lp-methods",
				title: "记录与方法",
				kicker: "数据"
			}, props.loading ? h$5(Skeleton, { height: 160 }) : h$5("div", { className: "lp-card" }, h$5("p", { className: "lp-muted" }, `方法和记录没有读到${props.error ? `（${props.error}）` : ""}。点右上角的刷新再试一次。`)));
			...
			return h$5(Section, {
				...
				title: "记录与方法",
				kicker: "数据",
				aside: h$5("span", { className: "lp-caption" }, `方法库 ${board.skills?.version ?? ""} · ${board.readiness?.declared ?? 0} 个个人方法`)
			}, h$5("div", { className: "lp-grid-2" }, h$5(RunReady, {
				board,
				onNotice: props.onNotice
			}), h$5("div", { className: "lp-card" }, h$5("div", { className: "lp-label" }, "再测一项就能解锁"), unlock.length === 0 ? h$5("p", { className: "lp-muted" }, "没有只差一项的方法。") : h$5("ul", { className: "lp-rows" }, ...unlock.slice(0, 8).map((row) => h$5("li", {
				key: row.item,
				className: "lp-row"
			}, h$5("span", { className: "lp-strong" }, row.item), h$5("span", { className: "lp-caption" }, `解锁 ${row.skills.length} 个方法`)))))), h$5("div", { className: "lp-grid-2" }, h$5("div", { className: "lp-card" }, h$5("div", { className: "lp-label" }, "用药计划", h$5("span", { className: "lp-optional" }, "只读，来自 Mirobody")), meds.length === 0 ? h$5("p", { className: "lp-muted" }, "没有读到用药计划。") : h$5("ul", { className: "lp-rows" }, ...meds.map((row) => h$5("li", {
				key: row.name,
				className: "lp-row"
			}, h$5("span", null, row.name), h$5("span", { className: "lp-caption" }, row.status ?? ""))))), h$5("div", { className: "lp-card" }, h$5("div", { className: "lp-label" }, "最近读出"), readouts.length === 0 ? h$5("p", { className: "lp-muted" }, "还没有算过。") : h$5("ul", { className: "lp-rows" }, ...readouts.slice(0, 8).map((row) => h$5("li", {
				key: row.key,
				className: "lp-row"
			}, h$5("span", null, row.label_zh || row.key), h$5("span", { className: "lp-row-end" }, h$5("span", { className: "lp-num" }, `${typeof row.value === "number" ? fmt(row.value, 2) : row.value ?? ""} ${row.unit && row.unit !== "1" ? row.unit === "a" ? "岁" : row.unit : ""}`), h$5("span", { className: "lp-caption" }, (row.measured_at || row.at || "").slice(0, 10)))))))), h$5("div", { className: "lp-grid-1" }, h$5(Search, { board })));
		}
		//#endregion
		...
		function TodayTile(props) {
			...
			return h$4("div", { className: "lp-card lp-tile lp-tile-today" }, h$4("div", { className: "lp-tile-head" }, h$4("div", { className: "lp-label" }, "今天"), items.length > 0 ? h$4("span", { className: "lp-caption" }, `${finished}/${items.length} 完成`) : null), items.length === 0 ? h$4("p", { className: "lp-muted" }, "今天没有需要打卡的项目，手环和 Mirobody 记录的会自动计入。") : h$4("ul", { className: "lp-today-list" }, ...items.map((row) => {
				const isDone = row.done_today || props.done.has(row.id);
				return h$4("li", {
					key: row.id,
					className: `lp-today-row ${isDone ? "lp-today-done" : ""}`
				}, h$4("span", { className: "lp-today-title" }, row.title), isDone ? h$4("span", { className: "lp-done-label" }, h$4(Icon, {
					name: "check",
					size: 14
				}), "已完成") : h$4(Btn, {
					size: "sm",
			...
			})), h$4("p", { className: "lp-fine" }, "也可以在对话里说一句“今天的方案我都完成了”。"));
		}
		function AdherenceTile(props) {
			...
			return h$4("div", { className: "lp-card lp-tile" }, h$4("div", { className: "lp-label" }, "方案执行"), h$4("div", { className: "lp-tile-row" }, h$4(Ring, {
				value: rate,
				label: "方案平均执行率",
				size: 56
			}), h$4("div", null, h$4("div", { className: "lp-tile-figure" }, rate == null ? "—" : `${Math.round(rate * 100)}%`), h$4("div", { className: "lp-caption" }, rate == null ? "还没有执行记录" : "近 12 周平均"))), streak > 1 ? h$4("div", { className: "lp-streak" }, h$4(Icon, {
				name: "flame",
				size: 15
			}), `连续 ${streak} 天`) : h$4("div", { className: "lp-caption lp-streak-empty" }, "连续完成两天以上会在这里显示"));
		}
		...
		function RetestTile(props) {
			...
			return h$4("div", { className: "lp-card lp-tile" }, h$4("div", { className: "lp-label" }, "下次复测"), now.length > 0 ? h$4("div", null, h$4("div", { className: "lp-tile-figure" }, "现在"), h$4("div", { className: "lp-caption" }, `可以复测${now.map((row) => row.marker).slice(0, 3).join("、")}`)) : first ? h$4("div", null, h$4("div", { className: "lp-tile-figure" }, `${daysBetween(props.today, first.date)} 天后`), h$4("div", { className: "lp-caption" }, `${chineseDate(first.date)}之后 · ${first.marker}`)) : h$4("div", null, h$4("div", { className: "lp-tile-figure lp-muted-ink" }, "—"), h$4("div", { className: "lp-caption" }, "保存方案后按指标排复测日")), h$4("p", { className: "lp-fine" }, h$4(Icon, {
				name: "calendar",
				size: 13
			}), " 复测太早，变化多半只是波动。"));
		}
		function Wins(props) {
			...
			if (wins.length === 0) return h$4("div", { className: "lp-card lp-wins lp-wins-empty" }, h$4("span", { className: "lp-win-icon lp-win-icon-quiet" }, h$4(Icon, {
				name: "spark",
				size: 16
			})), h$4("div", null, h$4("div", { className: "lp-strong" }, "还没有超出波动的改善"), h$4("div", { className: "lp-muted" }, "血脂、血糖、炎症指标通常要 1–3 个月才会动。坚持执行、按时复测，就是在积累证据。")));
			return h$4("div", { className: "lp-card lp-wins" }, h$4("div", { className: "lp-label" }, "真实的改善"), ...wins.map(({ item, row }, index) => h$4("div", {
				className: "lp-win",
				key: index,
				style: { animationDelay: `${index * 80}ms` }
			}, h$4("span", { className: "lp-win-icon" }, h$4(Icon, {
				name: "check",
				size: 16
			})), h$4("div", null, h$4("div", { className: "lp-strong" }, `${row.marker} ${fmtAuto(row.baseline?.value)} → ${fmtAuto(row.followup?.value)} ${row.unit ?? ""}`), h$4("div", { className: "lp-muted" }, `${item.title} · ${row.change ? pct(row.change.pct) : ""} · 超出个体正常波动`, (row.combined_with ?? []).length > 0 ? `（与${(row.combined_with ?? []).join("、")}共同作用）` : "")))));
		}
		...
		function ItemCard(props) {
			...
			return h$4("article", { className: "lp-card lp-item" }, h$4("div", { className: "lp-item-head" }, h$4("div", null, item.category_zh ? h$4("span", { className: "lp-cat" }, item.category_zh) : null, h$4("h3", { className: "lp-h3" }, item.title), h$4("div", { className: "lp-caption" }, `${item.start} 起 · 第 ${item.days ?? 0} 天`)), item.headline ? h$4(VerdictChip, { verdict: item.headline }) : null), h$4("div", { className: "lp-item-adherence" }, h$4("div", null, h$4("div", { className: "lp-caption" }, "近 12 周执行"), h$4("div", { className: "lp-item-figure" }, rate == null || adherence.level === "unknown" ? "记录不足" : `${Math.round(rate * 100)}%`), adherence.note_zh ? h$4("div", { className: "lp-fine lp-fine-tight" }, adherence.note_zh) : null), (adherence.calendar ?? []).length > 0 ? h$4(AdherenceStrip, {
				calendar: adherence.calendar ?? [],
				label: item.title
			}) : null), ...(item.verdicts ?? []).map((row, index) => h$4("div", {
				className: "lp-verdict",
				key: index
			}, h$4("div", { className: "lp-verdict-head" }, h$4(VerdictChip, { verdict: row.verdict }), h$4("span", { className: "lp-strong" }, row.marker), row.baseline && row.followup ? h$4("span", { className: "lp-num" }, `${fmtAuto(row.baseline.value)} → ${fmtAuto(row.followup.value)} ${row.unit ?? ""}${row.change ? `（${pct(row.change.pct)}）` : ""}`) : null), h$4("p", { className: "lp-reason" }, row.reason_zh ?? ""), (row.expected ?? []).length > 0 ? h$4("details", { className: "lp-expected" }, h$4("summary", null, "试验里平均能改变多少"), ...(row.expected ?? []).map((line) => h$4("p", {
				key: line.id,
				className: "lp-fine"
				...
				larger: "大于试验平均",
				opposite: "方向与试验相反"
			}[line.comparison] ?? ""}。` : "", ` doi:${line.doi}`))) : null)), h$4("div", { className: "lp-item-foot" }, idle ? h$4("span", { className: "lp-caption" }, idle) : source === "checkin" ? props.done ? h$4("span", { className: "lp-done-label lp-pop" }, h$4(Icon, {
				name: "check",
				size: 14
			}), "今天已完成") : h$4(Btn, {
				size: "sm",
				...
				name: "check",
				size: 14
			}), "今天完成了") : h$4("span", { className: "lp-caption" }, source === "wearable" ? "手环自动记录，不用打卡" : "服用情况在 Mirobody 里打卡")));
		}
		/** Stage plan: nothing saved yet. LongPi never proposes items; the person brings their own. */
		function PlanStart(props) {
			return h$4("div", { className: "lp-card lp-plan-start" }, h$4("div", { className: "lp-plan-start-text" }, h$4("h3", { className: "lp-h3" }, "想先改善哪一项？"), h$4("p", { className: "lp-muted" }, "说出你的方案，或上传医生、长寿师给的方案。LongPi 会读给你确认后保存，再按每个指标安排复测日，并算出达到目标时的模型估计。"), h$4("p", { className: "lp-fine" }, "LongPi 不替你制定方案，也不建议开始、停止或调整任何药物、补剂和剂量。")), h$4("div", { className: "lp-prompts" }, ...props.journey.suggestions.map((row) => h$4("button", {
				key: row.id,
				type: "button",
				className: "lp-prompt",
				onClick: () => props.onPrompt(row.text_zh)
			}, h$4("span", null, row.text_zh), h$4(Icon, {
				name: "arrow",
				size: 14
			})))));
		}
		function PlanSection(props) {
			...
			if (!props.journey.plan.exists && !tracking?.plan) return h$4(Section, {
				id: "lp-plan",
				title: "我的方案",
				kicker: "干预"
			}, h$4(PlanStart, {
			...
			if (props.loading && !tracking) return h$4(Section, {
				id: "lp-plan",
				title: props.journey.plan.title || "我的方案",
				kicker: "我的方案"
			}, h$4(Skeleton, { height: 260 }));
			...
			return h$4(Section, {
				...
				tracking,
				today
			})), h$4(Wins, { tracking }), items.length > 0 ? h$4("div", { className: "lp-card lp-timeline-card" }, h$4("div", { className: "lp-label" }, "时间线"), h$4(Timeline, {
				...
				checkups,
				today
			})) : null, items.length > 0 ? h$4("div", { className: "lp-grid-items" }, ...items.map((item) => h$4(ItemCard, {
				key: item.id,
				item,
		...
		function Markers(props) {
			...
			return h$4(Section, {
				...
				title: "指标变化",
				kicker: "对照正常波动"
			}, h$4("div", { className: "lp-grid-charts" }, ...charts.map((chart) => {
				...
				return h$4("figure", {
					...
					key: chart.indicator
				}, h$4("div", { className: "lp-figure-head" }, h$4("figcaption", null, h$4("span", { className: "lp-strong" }, chart.label), h$4("span", { className: "lp-caption" }, ` ${chart.unit}`)), verdict ? h$4(VerdictChip, { verdict: verdict.verdict }) : null), h$4(LineChart, {
					points: chart.points,
					unit: chart.unit,
					label: chart.label,
					height: 150,
				...
				}), h$4("p", { className: "lp-fine" }, chart.band ? `浅色带：以 ${chart.band.base_date} 的 ${fmt(chart.band.base, 2)} 为基线的正常波动范围${chart.band.verified === false ? "（变异数据待核对）" : ""}。落在带外才算真实变化。` : "缺少这项的个体变异数据，分不清真实变化和波动。"), h$4(TableTwin, {
		...
		function LongPiPage(props) {
			...
			return h$2("div", { className: "lp lp-page-root" }, h$2("div", { className: "lp-page" }, h$2(Header, {
				journey,
				refreshing,
				onRefresh: () => {
					doRefresh();
