window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function load(key, force = false) {
			...
			const seq = entry.seq;
			entry.loading = true;
			const run = getJson(force && REFRESHABLE.includes(key) ? `${PATHS[key]}?refresh=1` : PATHS[key]).then((data) => {
		...
		*/
		...
		function SmartDock(props) {
			...
			}, h$14("span", { className: "lp-dock-mark" }, h$14(Mark, { size: 14 }), "LongPi"), ...suggestions.slice(0, 3).map((row) => h$14("button", {
				key: row.id,
				type: "button",
				className: "lp-dock-pill",
				title: "放进输入框",
				onClick: () => place(row.text_zh)
			}, row.text_zh)), message ? h$14("span", {
		...
		function Tooltip(props) {
			...
			return h$13("div", {
				...
				role: "status"
			}, h$13("div", { className: "lp-tip-title" }, tip.title), ...tip.rows.map((row, index) => h$13("div", {
				className: "lp-tip-row",
				key: index
			}, h$13("span", { className: "lp-tip-value" }, row.value), h$13("span", { className: "lp-tip-label" }, row.label))));
		}
		function LineChart(props) {
			...
			if (points.length === 0) return h$13("div", {
				ref,
			...
			return h$13("div", {
			...
			}, ...yTicks.map((value) => h$13("g", { key: `g${value}` }, h$13("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(value),
				y2: y(value),
				...
				className: "lp-axis",
				textAnchor: "end"
			}, fmt(value, value % 1 === 0 ? 0 : digits)))), props.band ? h$13("rect", {
				x: bandFrom,
				...
				className: "lp-band",
				rx: 3
			}) : null, props.reference ? h$13("g", null, h$13("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(props.reference.value),
				y2: y(props.reference.value),
				className: "lp-ref"
			}), props.compact ? null : h$13("text", {
				x: width - pad.right + 4,
				y: y(props.reference.value) + 3,
				className: "lp-axis"
			}, props.reference.label)) : null, props.goal != null ? h$13("g", null, h$13("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(props.goal),
				y2: y(props.goal),
			...
			}, `目标 ${fmt(props.goal, digits)}`)) : null, h$13("path", {
				d: area,
				className: "lp-area"
			}), h$13("path", {
				d: path,
				className: "lp-line"
			}), focus != null ? h$13("line", {
				x1: x(days[focus]),
				x2: x(days[focus]),
				y1: pad.top,
				y2: height - pad.bottom,
				className: "lp-cross"
			}) : null, ...points.map((point, index) => points.length > 24 && index !== points.length - 1 && index !== focus ? null : h$13("circle", {
				key: `p${index}`,
				cx: x(days[index]),
				cy: y(point.value),
				r: focus === index ? 5.5 : 4,
				...
				y: y(last.value) + 4,
				className: "lp-end"
			}, `${fmt(last.value, digits)}`), props.compact ? null : h$13("text", {
				x: pad.left,
				y: height - 6,
				className: "lp-axis"
			}, monthLabel(points[0]?.date ?? "")), props.compact || points.length < 2 ? null : h$13("text", {
				x: width - pad.right,
				y: height - 6,
				className: "lp-axis",
				textAnchor: "end"
			}, monthLabel(last.date))), h$13(Tooltip, {
				tip,
				width
			}));
		}
		function Timeline(props) {
			...
			return h$13("div", {
			...
			}, ...months.filter((_, index) => index % step === 0).map((iso) => h$13("g", { key: iso }, h$13("line", {
				x1: x(dayNumber(iso)),
				x2: x(dayNumber(iso)),
				y1: 20,
				y2: height - 6,
				...
				y: 12,
				className: "lp-axis"
			}, monthLabel(iso)))), ...props.checkups.map((iso) => h$13("g", { key: `c${iso}` }, h$13("line", {
				x1: x(dayNumber(iso)),
				x2: x(dayNumber(iso)),
				y1: 22,
				y2: height - 6,
				...
				}, item.title.length > 12 ? `${item.title.slice(0, 11)}…` : item.title), h$13("rect", {
					x: labelWidth,
					y: y0 - 12,
			...
			})), h$13("div", { className: "lp-legend-inline" }, h$13("span", { className: "lp-key-bar" }), "执行中", h$13("span", { className: "lp-key-dot" }), "体检日"), h$13(Tooltip, {
				tip,
				width
			}));
		}
		function AdherenceStrip(props) {
			...
			}, ...props.calendar.map((day, index) => h$13("rect", {
				key: day.date,
		...
		function LeverBars(props) {
			...
			return h$13("div", {
				...
				return h$13("g", { key: item.label }, h$13("text", {
					x: 0,
					y: y0 - 2,
					className: "lp-row-label"
				}, item.label), h$13("text", {
					x: 0,
					y: y0 + 12,
					className: "lp-axis"
				}, item.detail), h$13("path", {
		...
		function TableTwin(props) {
			return h$13("details", { className: "lp-twin" }, h$13("summary", null, "表格"), h$13("table", null, h$13("caption", null, props.caption), h$13("thead", null, h$13("tr", null, ...props.head.map((cell) => h$13("th", {
				key: cell,
				scope: "col"
			}, cell)))), h$13("tbody", null, ...props.rows.map((row, index) => h$13("tr", { key: index }, ...row.map((cell, column) => h$13("td", { key: column }, cell)))))));
		}
		//#endregion
		...
		function Blocked(props) {
			...
			return h$10("div", { className: "lp-card lp-result lp-result-blocked" }, h$10(CardHead, { label: props.label }), h$10("div", { className: "lp-result-wait" }, "还不能计算"), h$10("p", { className: "lp-blocker" }, props.blocker || "还缺少计算需要的信息。"), props.needs.length > 0 ? h$10("div", { className: "lp-needs" }, h$10("span", { className: "lp-caption" }, "还需要"), ...props.needs.map((need) => h$10("span", {
				className: "lp-need",
				key: need
			}, need))) : null, props.selfAddon && props.selfAddon.self_key && props.action?.target !== "profile" && props.action?.target !== "records" ? h$10("div", { className: "lp-result-self" }, h$10("div", { className: "lp-caption" }, `${props.selfAddon.item_zh}可以自己在家量，记下就能算：`), h$10(InlineSelf, {
		...
		function FirstResult(props) {
			...
			return h$9("div", { className: "lp-step-body" }, !props.showResults && blocked === 0 ? h$9("p", { className: "lp-muted" }, "结果和还缺什么都写在下方的两张卡片里。") : null, !props.showResults ? null : h$9("div", { className: "lp-first" }, h$9("div", { className: "lp-first-cell" }, h$9("div", { className: "lp-caption" }, "身体年龄 · 模型估计"), bioage.status === "ok" ? h$9("div", null, h$9("div", { className: "lp-first-figure" }, fmt(bioage.phenoage), h$9("span", { className: "lp-bignum-unit" }, "岁")), h$9("div", { className: "lp-caption" }, [versusAge(bioage.advance), bioage.band_years != null ? `正常波动 ±${fmt(bioage.band_years)} 岁` : ""].filter(Boolean).join(" · "))) : h$9("p", { className: "lp-blocker" }, bioage.blocker_zh)), h$9("div", { className: "lp-first-cell" }, h$9("div", { className: "lp-caption" }, "10 年心血管病风险 · 模型估计"), risk.status === "ok" ? h$9("div", null, h$9("div", { className: "lp-first-figure" }, riskText(risk.risk_pct), h$9("span", { className: "lp-bignum-unit" }, "%")), h$9("div", { className: "lp-caption" }, [risk.category_zh, "China-PAR"].filter(Boolean).join(" · "))) : h$9("p", { className: "lp-blocker" }, risk.blocker_zh))), blocked > 0 ? h$9("div", { className: "lp-first-addons" }, h$9("div", { className: "lp-subhead" }, `还差 ${blocked} 项检查`, h$9("span", { className: "lp-optional" }, "下次体检加测，或现在自己量")), h$9(AddonList, {
				journey: props.journey,
				onNotice: props.onNotice,
		...
		function ProfileEditor(props) {
			...
			}, busy ? "保存中…" : onboarding ? "保存并继续" : "保存档案"), !onboarding && !dirty ? h$8("span", { className: "lp-caption" }, "已是最新") : null));
		}
		//#endregion
		...
		function RunReady(props) {
			...
			}, h$5("span", { className: "lp-strong" }, row.domain || row.name), h$5("span", { className: "lp-caption" }, row.blurb ?? "")))), ready.length > 8 ? h$5("p", { className: "lp-caption" }, `另有 ${ready.length - 8} 项`) : null, ready.length > 0 ? h$5("div", { className: "lp-form-actions" }, h$5(Btn, {
				size: "sm",
				disabled: running,
				onClick: () => {
					run();
		...
		function RetestTile(props) {
			...
			const upcoming = retests.filter((row) => row.date > props.today).sort((a, b) => a.date.localeCompare(b.date));
			const now = retests.filter((row) => row.date <= props.today);
			const first = upcoming[0];
			return h$4("div", { className: "lp-card lp-tile" }, h$4("div", { className: "lp-label" }, "下次复测"), now.length > 0 ? h$4("div", null, h$4("div", { className: "lp-tile-figure" }, "现在"), h$4("div", { className: "lp-caption" }, `可以复测${now.map((row) => row.marker).filter(Boolean).slice(0, 2).join("、")}`)) : first ? h$4("div", null, h$4("div", { className: "lp-tile-figure" }, `${daysBetween(props.today, first.date)} 天后`), h$4("div", { className: "lp-caption" }, `${chineseDate(first.date)}之后 · ${first.marker ?? ""}`)) : h$4("div", null, h$4("div", { className: "lp-tile-figure lp-muted-ink" }, "—"), h$4("div", { className: "lp-caption" }, "保存方案后按指标排复测日")), h$4("p", { className: "lp-fine" }, h$4(Icon, {
				name: "calendar",
				size: 13
			}), " 复测太早，变化多半只是波动。"));
		}
		...
		function ItemCard(props) {
			...
			return h$4("article", { className: "lp-card lp-item" }, h$4("div", { className: "lp-item-head" }, h$4("div", null, item.category_zh ? h$4("span", { className: "lp-cat" }, item.category_zh) : null, h$4("h3", { className: "lp-h3" }, item.title), h$4("div", { className: "lp-caption" }, `${item.start} 起 · 第 ${item.days ?? 0} 天`)), item.headline ? h$4(VerdictChip, { verdict: item.headline }) : null), h$4("div", { className: "lp-item-adherence" }, h$4("div", null, h$4("div", { className: "lp-caption" }, "近 12 周执行"), h$4("div", { className: "lp-item-figure" }, rate == null || adherence.level === "unknown" ? "记录不足" : `${Math.round(rate * 100)}%`), adherence.note_zh ? h$4("div", { className: "lp-fine lp-fine-tight" }, adherence.note_zh) : null), (adherence.calendar ?? []).length > 0 ? h$4(AdherenceStrip, {
				...
				larger: "大于试验平均",
				opposite: "方向与试验相反"
			}[line.comparison] ?? ""}。` : "", ` doi:${line.doi}`))) : null)), h$4("div", { className: "lp-item-foot" }, source === "checkin" ? props.done ? h$4("span", { className: "lp-done-label lp-pop" }, h$4(Icon, {
				name: "check",
				size: 14
			}), "今天已完成") : h$4(Btn, {
				size: "sm",
		...
		function JourneyStepper(props) {
			...
			return h$3("section", {
				...
				return h$3("li", {
					...
					size: 14,
					strokeWidth: 2
				}) : String(index + 1)), h$3("span", { className: "lp-stepbar-text" }, h$3("span", { className: "lp-stepbar-title" }, step.title), h$3("span", { className: "lp-stepbar-hint" }, state === "done" ? "已完成" : step.hint))));
			})), h$3("div", {
		...
		function ProfileAndSelf(props) {
			...
			return h$2(Section, {
				...
				className: "lp-card",
				id: "lp-profile-card"
			}, h$2("div", { className: "lp-label" }, "档案"), profileInStepper ? h$2("div", { className: "lp-pointer" }, h$2("p", { className: "lp-muted" }, props.journey.consent.accepted ? "档案在上方“建档”这一步填写，填好年龄和性别就能算身体年龄。" : "先在上方读一下说明并点“开始”，然后在同一处填写档案。"), h$2(Btn, {
				size: "sm",
				variant: "outline",
