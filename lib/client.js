	...
	factory: (require) => {
		...
		function Tooltip(props) {
			...
			return h$19("div", {
				...
				role: "status"
			}, h$19("div", { className: "lp-tip-title" }, tip.title), ...tip.rows.map((row, index) => h$19("div", {
				className: "lp-tip-row",
				key: index
			}, h$19("span", { className: "lp-tip-value" }, row.value), h$19("span", { className: "lp-tip-label" }, row.label))));
		}
		function LineChart(props) {
			...
			if (points.length === 0) return h$19("div", {
				ref,
			...
			return h$19("div", {
			...
			}, ...yTicks.map((value) => h$19("g", { key: `g${value}` }, h$19("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(value),
				y2: y(value),
				...
				className: "lp-axis",
				textAnchor: "end"
			}, fmt(value, value % 1 === 0 ? 0 : digits)))), props.band ? h$19("rect", {
				x: bandFrom,
				...
				className: "lp-band",
				rx: 3
			}) : null, props.reference ? h$19("g", null, h$19("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(props.reference.value),
				y2: y(props.reference.value),
				className: "lp-ref"
			}), props.compact ? null : h$19("text", {
				x: width - pad.right + 4,
				y: y(props.reference.value) + 3,
				className: "lp-axis"
			}, props.reference.label)) : null, props.goal != null ? h$19("g", null, h$19("line", {
				x1: pad.left,
				x2: width - pad.right,
				y1: y(props.goal),
				y2: y(props.goal),
			...
			}, `目标 ${fmt(props.goal, digits)}`)) : null, h$19("path", {
				d: area,
				className: "lp-area"
			}), h$19("path", {
				d: path,
				className: "lp-line"
			}), focus != null ? h$19("line", {
				x1: x(days[focus]),
				x2: x(days[focus]),
				y1: pad.top,
				y2: height - pad.bottom,
				className: "lp-cross"
			}) : null, ...points.map((point, index) => points.length > 24 && index !== points.length - 1 && index !== focus ? null : h$19("circle", {
				key: `p${index}`,
				cx: x(days[index]),
				cy: y(point.value),
				r: focus === index ? 5.5 : 4,
				...
				y: y(last.value) + 4,
				className: "lp-end"
			}, `${fmt(last.value, digits)}`), props.compact ? null : h$19("text", {
				x: pad.left,
				y: height - 6,
				className: "lp-axis"
			}, monthLabel(points[0]?.date ?? "")), props.compact || points.length < 2 ? null : h$19("text", {
				x: width - pad.right,
				y: height - 6,
				className: "lp-axis",
				textAnchor: "end"
			}, monthLabel(last.date))), h$19(Tooltip, {
				tip,
				width
			}));
		}
		function Timeline(props) {
			...
			return h$19("div", { className: "lp-timeline" }, h$19("div", {
				ref,
				className: "lp-chart",
				style: { height }
			}, h$19("svg", {
			...
			}, ...months.filter((_, index) => index % step === 0).map((iso) => h$19("g", { key: iso }, h$19("line", {
				x1: x(dayNumber(iso)),
				x2: x(dayNumber(iso)),
				y1: 20,
				y2: height - 6,
				...
				y: 12,
				className: "lp-axis"
			}, monthLabel(iso)))), ...props.checkups.map((iso) => h$19("g", { key: `c${iso}` }, h$19("line", {
				x1: x(dayNumber(iso)),
				x2: x(dayNumber(iso)),
				y1: 22,
				y2: height - 6,
				...
				}, item.title.length > 12 ? `${item.title.slice(0, 11)}…` : item.title), h$19("rect", {
					x: labelWidth,
					y: y0 - 12,
				...
				tip,
				width
			})), h$19("div", { className: "lp-legend-inline" }, h$19("span", { className: "lp-key-bar" }), "执行中", h$19("span", { className: "lp-key-dot" }), "体检日"));
		}
		function AdherenceStrip(props) {
			...
			}, ...props.calendar.map((day, index) => h$19("rect", {
				key: day.date,
		...
		function LeverBars(props) {
			...
			return h$19("div", {
				...
				return h$19("g", { key: item.label }, h$19("text", {
					x: 0,
					y: y0 + 14,
					className: "lp-row-label"
				}, item.label, item.detail ? h$19("tspan", {
					dx: 8,
					className: "lp-axis"
				}, item.detail) : null), h$19("path", {
		...
		function TableTwin(props) {
			return h$19("details", { className: "lp-twin" }, h$19("summary", null, "表格"), h$19("table", null, h$19("caption", null, props.caption), h$19("thead", null, h$19("tr", null, ...props.head.map((cell) => h$19("th", {
				key: cell,
				scope: "col"
			}, cell)))), h$19("tbody", null, ...props.rows.map((row, index) => h$19("tr", { key: index }, ...row.map((cell, column) => h$19("td", { key: column }, cell)))))));
		}
		//#endregion
		...
		function VerdictChip(props) {
			...
			return h$18("span", { className: `lp-chip-v ${style.className}` }, h$18(Icon, {
				name: style.icon,
				size: 14
			}), label);
		}
		...
		*/
		...
		function clearScrollRequest(id) {
		...
		function Section(props) {
			...
			}, h$17("div", { className: "lp-section-head" }, h$17("div", { className: "lp-section-titles" }, props.kicker ? h$17("div", { className: "lp-kicker" }, props.kicker) : null, h$17("h2", {
				className: "lp-h2",
				id: headingId
		...
		function Btn(props) {
			return h$17(_deepseek_ai_dsh_client_ui_primitives.Button, {
				variant: "primary",
				size: "md",
				...props
			});
		...
		function Segmented(props) {
			...
			return h$17("div", { className: "lp-seg-wrap" }, h$17("span", {
				id: labelId,
				className: props.hideLabel ? "lp-sr" : "lp-field-label"
			}, props.label), h$17("div", {
				className: "lp-seg",
				role: "radiogroup",
				"aria-labelledby": labelId
			}, ...props.options.map((option) => h$17("label", {
				key: option.value,
		...
		function Switch(props) {
			return h$17("button", {
				...
				"aria-hidden": true
			}, h$17("span", { className: "lp-switch-thumb" })), h$17("span", { className: "lp-switch-label" }, props.label));
		}
		...
		*/
		...
		function Evidence(props) {
			...
			return h$16("p", { className: "lp-evidence" }, h$16(Icon, {
				name: "flask",
				size: 13
			}), h$16("span", null, evidence.expected_zh || "有研究证据支持", population ? `（${population}）` : "", evidence.doi ? h$16(react.default.Fragment, null, " · ", h$16("a", {
		...
		function DraftItemCard(props) {
			...
			return h$16("li", { className: "lp-draft-item" }, h$16("div", { className: "lp-draft-item-head" }, h$16("div", { className: "lp-draft-item-title" }, item.category_zh ? h$16("span", { className: "lp-cat" }, item.category_zh) : null, h$16("span", { className: "lp-strong" }, item.title)), h$16("button", {
				type: "button",
				...
				name: "close",
				size: 12
			}), "去掉")), behaviorOf(item) ? h$16("p", { className: "lp-draft-detail" }, behaviorOf(item)) : null, item.target ? h$16("p", { className: "lp-caption lp-draft-target" }, h$16(Icon, {
				name: "check",
				size: 12
			}), " ", targetText(item.target)) : null, h$16(Evidence, { item }), warn ? h$16("div", { className: "lp-draft-warn" }, item.needs_doctor ? h$16("span", { className: "lp-warn-tag" }, h$16(Icon, {
				name: "warn",
				size: 12
			}), "需先与医生确认") : h$16(Icon, {
				name: "warn",
			...
			}), ...item.cautions_zh.map((text) => h$16("span", {
				key: text,
		...
		function Priorities(props) {
			...
			return h$16("div", { className: "lp-draft-block" }, h$16("div", { className: "lp-subhead" }, "为什么先改善这些", h$16("span", { className: "lp-optional" }, "按重要性排列")), h$16("ol", { className: "lp-priorities" }, ...rows.map((row, index) => h$16("li", {
				key: `${row.marker_key}-${index}`,
				className: "lp-priority"
			}, h$16("div", { className: "lp-priority-head" }, h$16("span", { className: "lp-strong" }, row.label_zh), row.value != null ? h$16("span", { className: "lp-num" }, `${num(row.value)} ${row.unit}`) : null), h$16("div", { className: "lp-caption" }, [row.why_zh, row.date ? `${chineseDate(row.date)}的记录` : ""].filter(Boolean).join(" · "))))));
		}
		function Goals$1(props) {
			if (props.goals.length === 0 && props.dropped === 0) return null;
			return h$16("div", { className: "lp-draft-block" }, h$16("div", { className: "lp-subhead" }, "目标", h$16("span", { className: "lp-optional" }, "模型估计")), props.goals.length > 0 ? h$16("ul", { className: "lp-rows lp-draft-goals" }, ...props.goals.map((goal) => h$16("li", {
				key: goal.marker,
				className: "lp-row"
			}, h$16("span", { className: "lp-row-main" }, h$16("span", { className: "lp-strong" }, goal.marker), h$16("span", { className: "lp-caption" }, `  ${goal.basis_zh}`)), h$16("span", { className: "lp-row-end lp-num" }, `${num(goal.value)} ${goal.unit}`)))) : null, props.dropped > 0 ? h$16("p", { className: "lp-fine" }, `去掉的项目对应的 ${props.dropped} 个目标也不会保存。`) : null);
		}
		function ConfirmModal(props) {
			const doctor = props.items.filter((item) => item.needs_doctor);
			return h$16(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				open: true,
				title: "采用这份方案",
				onClose: props.busy ? () => {} : props.onCancel,
				headless: true,
				className: "lp-confirm-dialog"
			}, h$16("div", { className: "lp lp-confirm" }, h$16("h2", { className: "lp-onb-title" }, "采用这份方案？"), h$16("p", { className: "lp-muted lp-confirm-lead" }, `保存为你的方案「${props.draft.title || "改善方案"}」，从今天（${chineseDate(props.today)}）开始。之后按项目打卡，并按每个指标安排复测；想调整随时在对话里说。`), h$16("ul", { className: "lp-confirm-list" }, ...props.items.map((item) => h$16("li", { key: item.id }, item.category_zh ? h$16("span", { className: "lp-cat" }, item.category_zh) : null, h$16("span", null, item.title), item.needs_doctor ? h$16("span", { className: "lp-warn-tag" }, "需先与医生确认") : null))), props.goals.length > 0 ? h$16("p", { className: "lp-caption" }, `目标：${props.goals.map((goal) => `${goal.marker} ${num(goal.value)} ${goal.unit}`).join("、")}（按试验平均效应估算，不是个人预测）`) : null, doctor.length > 0 ? h$16("p", { className: "lp-blocker lp-blocker-bad lp-confirm-doctor" }, `${doctor.map((item) => `「${item.title}」`).join("")}需先与医生确认后再开始。方案里不含任何剂量。`) : null, props.error ? h$16("p", {
				className: "lp-form-error",
				role: "alert"
			}, props.error) : null, h$16("div", { className: "lp-modal-actions" }, h$16(Btn, {
				variant: "outline",
		...
		function Hint(props) {
			return h$16("span", { className: "lp-caption lp-draft-hint" }, "想调整？在对话中说", h$16("button", {
				type: "button",
		...
		function Draft(props) {
			...
			return h$16("div", { className: "lp-card lp-draft" }, h$16("div", { className: "lp-draft-head" }, h$16("div", null, h$16("div", { className: "lp-kicker" }, "方案草稿 · 还没有保存"), h$16("h3", { className: "lp-h3 lp-draft-title" }, draft.title || "改善方案"), h$16("p", { className: "lp-muted" }, "LongPi 按你的检查结果和研究证据起草。每一项都注明试验里的平均效果，个人结果会不同；你确认后才保存。")), h$16("span", { className: "lp-tag" }, "草稿")), h$16(Priorities, { brief: data.brief }), h$16("div", { className: "lp-draft-block" }, h$16("div", { className: "lp-subhead" }, `建议的做法（${kept.length} 项）`), kept.length > 0 ? h$16("ul", { className: "lp-draft-items" }, ...kept.map((item) => h$16(DraftItemCard, {
				key: item.id,
				item,
				onRemove: () => toggle(item.id)
			}))) : h$16("p", { className: "lp-muted" }, "所有项目都去掉了。恢复一项，或在对话里说说你想怎么调整。"), gone.length > 0 ? h$16("div", { className: "lp-draft-removed" }, h$16("span", { className: "lp-caption" }, "已去掉："), ...gone.map((item) => h$16("button", {
				key: item.id,
				type: "button",
				className: "lp-toggle",
				...
				name: "plus",
				size: 12
			}), item.title))) : null), h$16(Goals$1, {
				goals,
				dropped: draft.goals.length - goals.length
			}), ...draft.notes_zh.map((text) => h$16("p", {
				key: text,
				className: "lp-fine"
			}, text)), h$16("div", { className: "lp-form-actions lp-draft-actions" }, h$16(Btn, {
				onClick: () => {
					setError(null);
					setConfirming(true);
				},
				disabled: kept.length === 0
			}, "采用这份方案"), h$16(Hint, { onPrompt: props.onPrompt })), h$16("p", { className: "lp-fine" }, data.brief.boundary_zh || "只起草生活方式；补剂只作为需先与医生确认的选项，不给剂量；不涉及任何处方药。"), confirming ? h$16(ConfirmModal, {
				draft,
				items: kept,
				goals,
		...
		function PlanDraftCard(props) {
			...
			if (!data && loading) return h$16("div", {
				className: "lp-card lp-draft",
				"aria-busy": true
			}, h$16("div", { className: "lp-kicker" }, "方案草稿"), h$16("p", { className: "lp-caption" }, "正在按你的结果和研究证据起草…"), h$16(Skeleton, { height: 72 }), h$16("div", { style: { height: 10 } }), h$16(Skeleton, { height: 72 }));
			if (!data) return h$16("div", { className: "lp-card lp-draft" }, h$16("div", { className: "lp-kicker" }, "方案草稿"), h$16("p", { className: "lp-muted" }, `没能读到方案草稿：${error ?? "没有返回"}。`), h$16("div", { className: "lp-form-actions" }, h$16(Hint, { onPrompt: props.onPrompt })));
			if (!data.draft) {
				...
				return h$16("div", { className: "lp-card lp-draft" }, h$16("div", { className: "lp-kicker" }, "方案草稿"), h$16("h3", { className: "lp-h3 lp-draft-title" }, "现在还起草不了方案"), h$16("p", { className: "lp-muted" }, reasons[0] || "你的记录里还没有能对上研究证据的指标。"), ...fine.map((text) => h$16("p", {
					key: text,
					className: "lp-fine"
				}, text)), h$16(Priorities, { brief: data.brief }), h$16("div", { className: "lp-form-actions lp-draft-actions" }, h$16(Hint, { onPrompt: props.onPrompt })));
			}
			return h$16(Draft, {
				data,
				draft: data.draft,
		...
		function TodayTile(props) {
			...
			return h$15("div", { className: "lp-card lp-tile lp-tile-today" }, h$15("div", { className: "lp-tile-head" }, h$15("div", { className: "lp-label" }, "今天"), items.length > 0 ? h$15("span", { className: "lp-caption" }, `${finished}/${items.length} 完成`) : null), items.length === 0 ? h$15("p", { className: "lp-muted" }, "今天没有需要打卡的项目，手环和 Mirobody 记录的会自动计入。") : h$15("ul", { className: "lp-today-list" }, ...items.map((row) => {
				const isDone = row.done_today || props.done.has(row.id);
				return h$15("li", {
					key: row.id,
					className: `lp-today-row ${isDone ? "lp-today-done" : ""}`
				}, h$15("span", { className: "lp-today-title" }, row.title), isDone ? h$15("span", { className: "lp-done-label" }, h$15(Icon, {
					name: "check",
					size: 14
				}), "已完成") : h$15(Btn, {
					size: "sm",
			...
			})), h$15("p", { className: "lp-fine" }, "也可以在对话里说一句“今天的方案我都完成了”。"));
		}
		function AdherenceTile(props) {
			...
			return h$15("div", { className: "lp-card lp-tile" }, h$15("div", { className: "lp-label" }, "方案执行"), h$15("div", { className: "lp-tile-row" }, h$15(Ring, {
				value: rate,
				label: "方案平均执行率",
				size: 56
			}), h$15("div", null, h$15("div", { className: "lp-tile-figure" }, rate == null ? "—" : `${Math.round(rate * 100)}%`), h$15("div", { className: "lp-caption" }, rate == null ? "还没有执行记录" : "近 12 周平均"))), streak > 1 ? h$15("div", { className: "lp-streak" }, h$15(Icon, {
				name: "flame",
				size: 15
			}), `连续 ${streak} 天`) : h$15("div", { className: "lp-caption lp-streak-empty" }, "连续完成两天以上会在这里显示"));
		}
		...
		function RetestTile(props) {
			...
			return h$15("div", { className: "lp-card lp-tile" }, h$15("div", { className: "lp-label" }, "下次复测"), now.length > 0 ? h$15("div", null, h$15("div", { className: "lp-tile-figure" }, "现在"), h$15("div", { className: "lp-caption" }, `可以复测${now.map((row) => row.marker).slice(0, 3).join("、")}`)) : first ? h$15("div", null, h$15("div", { className: "lp-tile-figure" }, `${daysBetween(props.today, first.date)} 天后`), h$15("div", { className: "lp-caption" }, `${chineseDate(first.date)}之后 · ${first.marker}`)) : h$15("div", null, h$15("div", { className: "lp-tile-figure lp-muted-ink" }, "—"), h$15("div", { className: "lp-caption" }, "保存方案后按指标排复测日")), h$15("p", { className: "lp-fine" }, h$15(Icon, {
				name: "calendar",
				size: 13
			}), " 复测太早，变化多半只是波动。"));
		}
		function Wins(props) {
			...
			if (wins.length === 0) return h$15("div", { className: "lp-card lp-wins lp-wins-empty" }, h$15("span", { className: "lp-win-icon lp-win-icon-quiet" }, h$15(Icon, {
				name: "spark",
				size: 16
			})), h$15("div", null, h$15("div", { className: "lp-strong" }, "还没有超出波动的改善"), h$15("div", { className: "lp-muted" }, "血脂、血糖、炎症指标通常要 1–3 个月才会动。坚持执行、按时复测，就是在积累证据。")));
			return h$15("div", { className: "lp-card lp-wins" }, h$15("div", { className: "lp-label" }, "真实的改善"), ...wins.map(({ item, row }, index) => h$15("div", {
				className: "lp-win",
				key: index,
				style: { animationDelay: `${index * 80}ms` }
			}, h$15("span", { className: "lp-win-icon" }, h$15(Icon, {
				name: "check",
				size: 16
			})), h$15("div", null, h$15("div", { className: "lp-strong" }, `${row.marker} ${fmtAuto(row.baseline?.value)} → ${fmtAuto(row.followup?.value)} ${row.unit ?? ""}`), h$15("div", { className: "lp-muted" }, `${item.title} · ${row.change ? pct(row.change.pct) : ""} · 超出个体正常波动`, (row.combined_with ?? []).length > 0 ? `（与${(row.combined_with ?? []).join("、")}共同作用）` : "")))));
		}
		...
		function ItemCard(props) {
			...
			return h$15("article", { className: "lp-card lp-item" }, h$15("div", { className: "lp-item-head" }, h$15("div", null, item.category_zh ? h$15("span", { className: "lp-cat" }, item.category_zh) : null, h$15("h3", { className: "lp-h3" }, item.title), h$15("div", { className: "lp-caption" }, `${item.start} 起 · 第 ${item.days ?? 0} 天`)), item.headline ? h$15(VerdictChip, { verdict: item.headline }) : null), h$15("div", { className: "lp-item-adherence" }, h$15("div", null, h$15("div", { className: "lp-caption" }, "近 12 周执行"), h$15("div", { className: "lp-item-figure" }, rate == null || adherence.level === "unknown" ? "记录不足" : `${Math.round(rate * 100)}%`), adherence.note_zh ? h$15("div", { className: "lp-fine lp-fine-tight" }, adherence.note_zh) : null), (adherence.calendar ?? []).length > 0 ? h$15(AdherenceStrip, {
				calendar: adherence.calendar ?? [],
				label: item.title
			}) : null), ...(item.verdicts ?? []).map((row, index) => h$15("div", {
				className: "lp-verdict",
				key: index
			}, h$15("div", { className: "lp-verdict-head" }, h$15(VerdictChip, { verdict: row.verdict }), h$15("span", { className: "lp-strong" }, row.marker), row.baseline && row.followup ? h$15("span", { className: "lp-num" }, `${fmtAuto(row.baseline.value)} → ${fmtAuto(row.followup.value)} ${row.unit ?? ""}${row.change ? `（${pct(row.change.pct)}）` : ""}`) : null), h$15("p", { className: "lp-reason" }, row.reason_zh ?? ""), (row.expected ?? []).length > 0 ? h$15("details", { className: "lp-expected" }, h$15("summary", null, "试验里平均能改变多少"), ...(row.expected ?? []).map((line) => h$15("p", {
				key: line.id,
				className: "lp-fine"
				...
				larger: "大于试验平均",
				opposite: "方向与试验相反"
			}[line.comparison] ?? ""}。` : "", ` doi:${line.doi}`))) : null)), h$15("div", { className: "lp-item-foot" }, idle ? h$15("span", { className: "lp-caption" }, idle) : source === "checkin" ? props.done ? h$15("span", { className: "lp-done-label lp-pop" }, h$15(Icon, {
				name: "check",
				size: 14
			}), "今天已完成") : h$15(Btn, {
				size: "sm",
				...
				name: "check",
				size: 14
			}), "今天完成了") : h$15("span", { className: "lp-caption" }, source === "wearable" ? "手环自动记录，不用打卡" : "服用情况在 Mirobody 里打卡")));
		}
		/** Stage plan, next to the draft: the person may bring their own plan instead. */
		function PlanStart(props) {
			return h$15("div", { className: "lp-card lp-plan-start" }, h$15("div", { className: "lp-plan-start-text" }, h$15("h3", { className: "lp-h3" }, "已经有自己的方案？"), h$15("p", { className: "lp-muted" }, "说出你的方案，或上传医生、长寿师给的方案。LongPi 会读给你确认后保存，再按每个指标安排复测日，并算出达到目标时的模型估计。"), h$15("p", { className: "lp-fine" }, "LongPi 只起草生活方式方案；不会开始、停止或调整任何处方药，也不给药物或补剂的剂量。")), h$15("div", { className: "lp-prompts" }, ...props.journey.suggestions.map((row) => h$15("button", {
				key: row.id,
				type: "button",
				className: "lp-prompt",
				onClick: () => props.onPrompt(row.text_zh)
			}, h$15("span", null, row.text_zh), h$15(Icon, {
				name: "arrow",
				size: 14
			})))));
		}
		function PlanSection(props) {
			...
			if (!props.journey.plan.exists && !tracking?.plan) return h$15(Section, {
				id: "lp-plan",
				title: "我的方案",
				kicker: "干预"
			}, h$15(PlanDraftCard, {
			...
			if (props.loading && !tracking) return h$15(Section, {
				id: "lp-plan",
				title: props.journey.plan.title || "我的方案",
				kicker: "我的方案"
			}, h$15(Skeleton, { height: 260 }));
			...
			return h$15(Section, {
				...
				tracking,
				today
			})), h$15(Wins, { tracking }), items.length > 0 ? h$15("div", { className: "lp-card lp-timeline-card" }, h$15("div", { className: "lp-label" }, "时间线"), h$15(Timeline, {
				...
				checkups,
				today
			})) : null, items.length > 0 ? h$15("div", { className: "lp-grid-items" }, ...items.map((item) => h$15(ItemCard, {
				key: item.id,
				item,
		...
		function Markers(props) {
			...
			return h$15(Section, {
				...
				title: "指标变化",
				kicker: "对照正常波动"
			}, h$15("div", { className: "lp-grid-charts" }, ...charts.map((chart) => {
				...
				return h$15("figure", {
					...
					key: chart.indicator
				}, h$15("div", { className: "lp-figure-head" }, h$15("figcaption", null, h$15("span", { className: "lp-strong" }, chart.label), h$15("span", { className: "lp-caption" }, ` ${chart.unit}`)), verdict ? h$15(VerdictChip, { verdict: verdict.verdict }) : null), h$15(LineChart, {
					points: chart.points,
					unit: chart.unit,
					label: chart.label,
					height: 150,
				...
				}), h$15("p", { className: "lp-fine" }, chart.band ? `浅色带：以 ${chart.band.base_date} 的 ${fmt(chart.band.base, 2)} 为基线的正常波动范围${chart.band.verified === false ? "（变异数据待核对）" : ""}。落在带外才算真实变化。` : "缺少这项的个体变异数据，分不清真实变化和波动。"), h$15(TableTwin, {
		...
		*/
		...
		function RetestPart(props) {
			if (!props.text) return props.before ? h$14(Sep$1) : null;
			return h$14(react.default.Fragment, null, props.before ? h$14(Sep$1) : null, h$14("span", null, props.text), h$14(Sep$1));
		}
		...
		function RoutineRow(props) {
			...
			react.default.useEffect(() => {
				setDone((current) => {
					...
					return current.day === journey.today && ids.length === current.ids.length ? current : {
						day: journey.today,
						ids
					};
				});
			...
			const checkIn = (id, title) => {
				...
				postJson("/api/longpi/checkin", {
					...
					setDone((current) => ({
						day: journey.today,
						ids: [...current.day === journey.today ? current.ids : [], id]
					}));
					notifyChanged();
			...
			if (items.length === 0 && journey.next.detail_zh) lead.push(h$14("span", { key: "next" }, journey.next.detail_zh));
			return h$14("div", {
				className: "lp lp-home-row",
				role: "group",
				"aria-label": "LongPi 今天"
			}, ...lead, reminder?.date ? h$14(RetestPart, {
			...
			}, "健康页 →"), h$14(Note, { text: failed ?? props.note }));
		}
		...
		function HomeRow(props) {
			...
			if (journey.stage === "routine") return h$14(RoutineRow, {
				journey,
			...
			}, ...suggestions.map((row) => h$14("button", {
				key: row.id,
				type: "button",
				className: "lp-suggest",
				title: "放进输入框",
				onClick: () => setPendingPrompt(row.text_zh, "hero")
			}, row.text_zh)), h$14(Note, { text: note }));
		}
		//#endregion
		...
		*/
		...
		function Status(props) {
			...
			const parts = [];
			if (journey.stage === "consent" || journey.stage === "profile") parts.push("花 2 分钟建档，算出你的身体年龄和心血管风险", h$13(Sep, { key: "s" }), h$13(Go, {
				key: "go",
				label: "开始建档",
				onClick: open
			}));
			else if (journey.stage === "records") parts.push(journey.records.status === "error" ? "体检记录读取失败，暂时算不出结果" : "连接体检记录后，就能算出你的身体年龄", h$13(Sep, { key: "s" }), h$13(Go, {
				key: "go",
				...
				parts.push("还差 ", h$13("b", { key: "n" }, `${journey.addons.length} 项检查`), `就能算出${joinUnlocks(journey)}：${named}${more}`, h$13(Sep, { key: "s" }), h$13(Go, {
					key: "go",
					label: "加测清单",
					onClick: open
				}));
				...
				if (rows.length > 0) rows.forEach((row, index) => {
					if (index > 0) parts.push(h$13(Sep, { key: `s${index}` }));
					parts.push(h$13(react.default.Fragment, { key: `f${index}` }, ...row));
				});
				else {
					...
					else parts.push(detail, h$13(Sep, { key: "s" }), h$13(Go, {
						key: "go",
			...
			return h$13("p", { className: "lp-hero-status" }, ...parts);
		}
		/**
		...
		*/
		function ChangesLine(props) {
			...
			return h$13("p", { className: "lp-hero-changes" }, h$13(Icon, {
				name: "warn",
				size: 14,
				className: "lp-hero-changes-icon"
			}), `${subject}的变化超出正常波动`, h$13(Sep), h$13(Go, {
				label: "查看",
		...
		function Hero(props) {
			...
			return h$13("div", {
				...
				role: "group",
				"aria-label": "LongPi"
			}, h$13("div", { className: "lp-hero-title" }, h$13("span", {
				...
				size: 16,
				strokeWidth: 1.8
			})), h$13("span", null, titleOf(props.journey))), h$13(Status, {
				journey: props.journey,
				open: props.open
			}), h$13(ChangesLine, {
				journey: props.journey,
				openAt: props.openAt
			}), inline ? h$13("div", { className: "lp-hero-row" }, row) : null, host ? (0, react_dom.createPortal)(row, host) : null);
		}
		...
		function TimeField(props) {
			return h$12("div", { className: "lp-field" }, h$12("label", {
				className: "lp-field-label",
				htmlFor: props.id
			}, props.label, props.hint ? h$12("span", { className: "lp-optional" }, props.hint) : null), h$12("input", {
				id: props.id,
				type: "time",
		...
		function Log(props) {
			...
			return h$12("div", { className: "lp-followup-log" }, h$12("div", { className: "lp-subhead" }, "最近发送", h$12("span", { className: "lp-optional" }, rows.length > 0 ? `最近 ${rows.length} 条` : "")), rows.length === 0 ? h$12("p", { className: "lp-caption" }, "还没有发过提醒。") : h$12("ul", { className: "lp-rows" }, ...rows.map((row, index) => h$12("li", {
				key: `${row.at}-${index}`,
				className: "lp-row"
			}, h$12("span", { className: "lp-row-main" }, h$12("span", { className: "lp-num" }, whenText(row.at)), "  ", h$12("span", null, LOG_ZH[row.kind] ?? row.kind), row.error ? h$12("span", { className: "lp-caption" }, `  ${row.error}`) : null), h$12("span", { className: "lp-row-end" }, channelsText(row) ? h$12("span", { className: "lp-caption" }, channelsText(row)) : null, h$12("span", { className: `lp-sent ${row.ok ? "lp-sent-ok" : "lp-sent-bad"}` }, h$12(Icon, {
		...
		function TestResult(props) {
			...
			if (rows.length === 0) return h$12("span", {
		...
		function Settings(props) {
			...
			return h$12("div", { className: "lp-card lp-followup" }, h$12("div", { className: "lp-followup-head" }, h$12(Switch, {
				id: "lp-followup-on",
			...
			}), h$12("span", { className: "lp-caption" }, settings.enabled ? hasChannel ? nextLine(data) : "已开启，但还没有可用的渠道：打开桌面通知或填写 Webhook。" : "关闭时不会发送任何提醒。开启后按下面的时间提醒打卡、到期复测和每周小结。")), h$12("div", { className: "lp-grid-2 lp-followup-grid" }, h$12("fieldset", { className: "lp-fieldset" }, h$12("legend", { className: "lp-label" }, "什么时候"), h$12("div", { className: "lp-followup-times" }, h$12(TimeField, {
				id: "lp-fu-checkin",
				label: "打卡提醒",
				hint: "当天还有未完成时",
			...
			})), h$12("div", { className: "lp-field" }, h$12("label", {
				className: "lp-field-label",
				htmlFor: "lp-fu-weekly"
			}, "每周小结"), h$12("div", { className: "lp-input-unit" }, h$12("select", {
				id: "lp-fu-weekly",
				className: "lp-select lp-select-wide",
				value: form.weeklyDay,
				onChange: (event) => set("weeklyDay", event.target.value)
			}, h$12("option", { value: "0" }, "不发送"), ...DAYS.map((label, index) => h$12("option", {
				key: label,
				value: String(index + 1)
			}, label))), form.weeklyDay !== "0" ? h$12("input", {
				type: "time",
				className: "lp-input lp-input-time",
			...
			}) : null)), h$12("div", { className: "lp-field" }, h$12(Check, {
				id: "lp-fu-quiet",
				checked: form.quietOn,
				onChange: (checked) => set("quietOn", checked)
			}, "免打扰时段"), form.quietOn ? h$12("div", { className: "lp-input-unit" }, h$12("input", {
				type: "time",
				className: "lp-input lp-input-time",
			...
			}), h$12("span", { className: "lp-unit" }, "至"), h$12("input", {
				type: "time",
				className: "lp-input lp-input-time",
				value: form.quietEnd,
				"aria-label": "免打扰结束",
			...
			})) : null)), h$12("fieldset", { className: "lp-fieldset" }, h$12("legend", { className: "lp-label" }, "发到哪里"), h$12(Check, {
				id: "lp-fu-desktop",
				checked: form.desktop && data.platform_desktop,
				disabled: !data.platform_desktop,
				onChange: (checked) => set("desktop", checked)
			}, "桌面通知", h$12("span", { className: "lp-caption" }, data.platform_desktop ? "  这台电脑的系统通知" : "  这台电脑的系统不支持")), h$12("div", { className: "lp-field" }, h$12("label", {
				className: "lp-field-label",
				htmlFor: "lp-fu-kind"
			}, "Webhook 渠道", h$12("span", { className: "lp-optional" }, "可选：发到手机上的群机器人或 App")), h$12("select", {
				id: "lp-fu-kind",
				className: "lp-select lp-select-wide",
				value: form.kind,
			...
			}, h$12("option", { value: "" }, "不使用"), ...Object.keys(KIND_ZH).map((key) => h$12("option", {
				key,
				value: key
			}, KIND_ZH[key])))), kind ? h$12("div", { className: "lp-field" }, h$12("label", {
				className: "lp-field-label",
				htmlFor: "lp-fu-url"
			}, "地址", storedSame ? h$12("span", { className: "lp-optional" }, `已设置：${settings.webhook?.url_masked}`) : null), h$12("input", {
				id: "lp-fu-url",
				type: "url",
				className: "lp-input",
				value: form.url,
			...
			})) : null, kind && SIGNED.includes(kind) ? h$12("div", { className: "lp-field" }, h$12("label", {
				className: "lp-field-label",
				htmlFor: "lp-fu-secret"
			}, "签名密钥", h$12("span", { className: "lp-optional" }, "机器人开了“加签”才需要")), h$12("div", { className: "lp-input-unit" }, h$12("input", {
				id: "lp-fu-secret",
				type: "password",
				className: "lp-input",
				value: form.secret,
			...
			}), storedSame && settings.webhook?.secret_set && !form.clearSecret && !form.secret ? h$12("button", {
				type: "button",
				className: "lp-linkbtn",
				onClick: () => set("clearSecret", true)
			}, "清除") : null)) : null)), h$12("div", { className: "lp-followup-detail" }, h$12(Segmented, {
				name: "lp-fu-detail",
				label: "内容",
				value: form.detail,
			...
			}), h$12("p", { className: "lp-caption" }, form.detail === "minimal" ? "只发“今天还有 2 项待打卡”这类提示，不含项目名称和健康数值。" : "会带上方案项目名称、执行率和复测指标，发到你配置的渠道。")), error ? h$12("p", {
				className: "lp-form-error",
				role: "alert"
			}, error) : null, h$12("div", { className: "lp-form-actions" }, h$12(Btn, {
				onClick: () => {
					save();
				},
				disabled: !dirty || saving
			}, saving ? "保存中…" : "保存设置"), h$12(Btn, {
				variant: "outline",
				onClick: () => {
					sendTest();
				},
				...
				name: "send",
				size: 14
			}), testing ? "发送中…" : "发送测试"), dirty ? h$12("span", { className: "lp-caption" }, "有未保存的更改") : test ? h$12(TestResult, {
				result: test,
				kind: settings.webhook?.kind ?? null
			}) : null), h$12("p", { className: "lp-fine" }, FOLLOWUP_NOTE), h$12(Log, { rows: data.log }));
		}
		function FollowupSection(props) {
			const { data, loading, error } = useFollowup();
			let body;
			if (!data && loading) body = h$12(Skeleton, {
				height: 220,
				className: "lp-card-skeleton"
			});
			else if (!data) body = h$12("div", { className: "lp-card" }, h$12("p", { className: "lp-muted" }, `随访设置没有读到：${error ?? "没有返回"}。LongPi 插件可能需要更新。`));
			else body = h$12(Settings, {
				data,
				onNotice: props.onNotice
			});
			return h$12(Section, {
		...
		function FollowupOptIn() {
			...
			return h$12("div", { className: "lp-optin" }, h$12(Check, {
				id: "lp-onb-followup",
			...
			}, `每天${Number(settings.checkin_time.slice(0, 2)) >= 17 ? "晚上" : ""} ${settings.checkin_time} 提醒我打卡（桌面通知）`), h$12("span", { className: "lp-caption" }, data.platform_desktop ? "只在 DSH 运行时提醒，不含健康数值；随时可以在健康页的“随访提醒”里关闭或改时间。" : "这台电脑的系统不支持桌面通知；可以在健康页的“随访提醒”里设置 Webhook。"), error ? h$12("span", {
				className: "lp-form-error",
				role: "alert"
			}, error) : null);
		}
		...
		function SelfLatestList(props) {
			...
			if (rows.length === 0) return null;
			return h$11("ul", { className: "lp-self-latest" }, ...rows.map((row) => h$11("li", { key: row.key }, h$11("span", { className: "lp-caption" }, row.key === "sbp" ? "家庭血压" : row.label_zh), h$11("span", { className: "lp-self-value" }, selfLatestText(row, props.latest)), h$11("span", { className: "lp-caption" }, row.key === "sbp" ? `${row.n > 1 ? `7 天均值 · ${row.n} 次` : "1 次读数"} · 截至 ${chineseDate(row.date)}` : chineseDate(row.date)))));
		}
		...
		function UnitSelect(props) {
			...
			if (choices.length < 2) return h$11("span", { className: "lp-unit" }, props.spec.unit);
			return h$11("select", {
				id: props.id,
		...
		function SelfMeasureForm(props) {
			...
			const field = (key, placeholder) => h$11("div", { className: "lp-self-field" }, h$11("label", {
				className: "lp-field-label",
				htmlFor: `${props.idPrefix}-${key}`
			}, spec(key).label_zh), h$11("div", { className: "lp-input-unit" }, h$11("input", {
			...
			return h$11("form", {
				...
				noValidate: true
			}, h$11("div", { className: "lp-self-grid" }, field("waist", "例如 86"), field("weight", "例如 70.5"), h$11("div", { className: "lp-self-field lp-self-bp" }, h$11("span", {
				className: "lp-field-label",
			...
			}), h$11("span", { className: "lp-unit" }, "mmHg"))), h$11("div", { className: "lp-self-field" }, h$11("label", {
				className: "lp-field-label",
				...
				type: "submit",
				disabled: busy
			}, busy ? "记录中…" : "记录"), h$11("span", { className: "lp-caption" }, "单位可以选斤、尺或寸，会换算成 kg 和 cm。")));
		}
		function SelfRecent(props) {
			...
			if (rows.length === 0) return h$11("p", { className: "lp-caption lp-self-empty" }, "还没有自测记录。");
			...
			return h$11("div", { className: "lp-self-recent" }, h$11("div", { className: "lp-subhead" }, "最近记录"), h$11("ul", { className: "lp-rows" }, ...rows.map((row) => h$11("li", {
				key: row.id,
				className: "lp-row"
			}, h$11("span", { className: "lp-row-main" }, h$11("span", null, labelOf(row.key)), h$11("span", { className: "lp-num" }, ` ${fmt(row.value)} ${row.unit}`), row.given ? h$11("span", { className: "lp-caption" }, `（记为 ${fmt(row.given.value)} ${row.given.unit}）`) : null), h$11("span", { className: "lp-caption" }, chineseDate(row.date)), h$11("button", {
				type: "button",
		...
		function InlineSelf(props) {
			...
			}) : null, bp ? h$11("span", { className: "lp-unit" }, "mmHg") : h$11(UnitSelect, {
		...
		function CardHead(props) {
			return h$10("div", { className: "lp-result-head" }, h$10("div", { className: "lp-label" }, props.label), h$10(EstimateTag));
		}
		...
		function Blocked(props) {
			...
			return h$10("div", { className: "lp-card lp-result lp-result-blocked" }, h$10(CardHead, { label: props.label }), h$10("div", { className: "lp-result-wait" }, "还不能计算"), h$10("p", { className: "lp-blocker" }, props.blocker || "还缺少计算需要的信息。"), props.needs.length > 0 ? h$10("div", { className: "lp-needs" }, h$10("span", { className: "lp-caption" }, "还需要"), ...props.needs.slice(0, NEEDS_SHOWN).map((need) => h$10("span", {
				className: "lp-need",
				key: need
			}, need)), props.needs.length > NEEDS_SHOWN ? h$10("span", { className: "lp-caption" }, `等 ${props.needs.length} 项`) : null) : null, props.selfAddon && props.selfAddon.self_key && props.action?.target !== "profile" && props.action?.target !== "records" ? h$10("div", { className: "lp-result-self" }, h$10("div", { className: "lp-caption" }, `${props.selfAddon.item_zh}可以自己在家量，记下就能算：`), h$10(InlineSelf, {
				journey: props.journey,
				selfKey: props.selfAddon.self_key,
				idPrefix: `${props.idPrefix}-self`,
				onNotice: props.onNotice
			})) : action ? h$10("div", { className: "lp-result-action" }, h$10(Btn, {
				size: "sm",
				variant: "outline",
				onClick: () => props.onAction(action.target)
			}, action.label, h$10(Icon, {
				name: "arrow",
				size: 14
			}))) : null);
		}
		function BodyAgeCard(props) {
			...
			if (result.status !== "ok") return h$10(Blocked, {
				journey: props.journey,
				label: "身体年龄 · 表型年龄",
			...
			return h$10("div", { className: `lp-card lp-result ${real ? "lp-result-win" : ""}` }, h$10(CardHead, { label: "身体年龄 · 表型年龄" }), h$10("div", { className: "lp-result-figure" }, h$10("span", { className: "lp-bignum" }, fmt(phenoage)), h$10("span", { className: "lp-bignum-unit" }, "岁"), band != null ? h$10("span", { className: "lp-band-note" }, `正常波动 ±${fmt(band)} 岁`) : null), h$10("div", { className: "lp-result-sub" }, versus ? h$10("span", { className: `lp-pill ${advance != null && advance < -.5 ? "lp-pill-good" : ""}` }, versus) : null, date ? h$10("span", { className: "lp-caption" }, `${chineseDate(date)}体检 · 共 ${count} 次完整血检`) : null), result.caveat_zh ? h$10("p", {
				className: "lp-caveat",
				role: "note"
			}, h$10(Icon, {
				name: "warn",
				size: 14
			}), h$10("span", null, result.caveat_zh)) : null, points.length > 1 ? h$10(LineChart, {
			...
			}) : props.tracking == null ? h$10(Skeleton, { height: 60 }) : null, story ? h$10("p", { className: "lp-story" }, real ? h$10(Icon, {
				name: "spark",
				className: "lp-good-ink"
			}) : null, story) : null, h$10("p", { className: "lp-fine" }, "Levine 2018 表型年龄：九项常规血检加实足年龄。浅色带是第一次检查的个体正常波动，落在带外才算真实变化。"), points.length > 1 ? h$10(TableTwin, {
				caption: "每次体检的表型年龄",
				head: [
					"日期",
					"表型年龄",
		...
		function RiskCard(props) {
			...
			return h$10("div", { className: "lp-card lp-result" }, h$10(CardHead, { label: "10 年心血管病风险 · China-PAR" }), h$10("div", { className: "lp-result-figure" }, h$10("span", { className: "lp-bignum" }, riskText(result.risk_pct)), h$10("span", { className: "lp-bignum-unit" }, "%")), h$10("div", { className: "lp-result-sub" }, result.category_zh ? h$10("span", { className: "lp-pill" }, result.category_zh) : null, result.date ? h$10("span", { className: "lp-caption" }, `按 ${chineseDate(result.date)}的记录和你的档案计算`) : null), goal != null && Number.isFinite(goal) ? h$10("div", { className: "lp-result-goal" }, h$10("span", { className: "lp-caption" }, "达到方案目标时"), h$10("span", { className: "lp-strong" }, `${riskText(goal)}%`), card?.category_zh?.goal ? h$10("span", { className: "lp-pill lp-pill-good" }, card.category_zh.goal) : null) : null, h$10("p", { className: "lp-story" }, "未来 10 年发生心梗、脑卒中等动脉粥样硬化性心血管病的估计概率。它说的是同类人群的平均，不是对你个人的预言。"), h$10("p", { className: "lp-fine" }, `China-PAR 中国人群模型。${card?.note_zh ?? ""}这个模型没有公开的个体波动范围，所以这里不画波动带。`));
		}
		function ResultsRow(props) {
			...
			const bio = h$10(BodyAgeCard, {
				key: "bio",
				...props
			});
			const risk = h$10(RiskCard, {
				key: "risk",
				...props
			});
			return h$10("div", {
		...
		function AddonList(props) {
			...
			if (addons.length === 0) return h$10("p", { className: "lp-muted" }, "没有需要加测的项目。");
			return h$10("ul", {
				className: "lp-addons",
				id: `${props.idPrefix}-addons`
			}, ...addons.map((row) => h$10("li", {
				key: row.item_zh,
				className: "lp-addon"
			}, h$10("span", {
				...
				size: 14
			})), h$10("div", { className: "lp-addon-text" }, h$10("div", { className: "lp-strong" }, row.item_zh), h$10("div", { className: "lp-caption" }, `解锁：${row.unlocks_zh}${row.self_measurable ? " · 可以自己在家量" : " · 下次体检加测"}`)), row.self_measurable && row.self_key ? h$10(InlineSelf, {
				journey: props.journey,
		...
		function ConsentText() {
			return h$9("div", { className: "lp-consent" }, ...CONSENT_SENTENCES.map((text, index) => h$9("div", {
				key: index,
		...
		function ConsentInline(props) {
			...
			return h$9("div", { className: "lp-step-body" }, h$9(ConsentText), h$9("div", { className: "lp-form-actions" }, h$9(Btn, {
				onClick: () => {
					accept();
				},
				disabled: busy
			}, busy ? "记录中…" : "开始"), h$9("span", { className: "lp-caption" }, `点“开始”表示你已读过以上说明（版本 ${props.journey.consent.current}）。`)));
		}
		...
		function RecordsGuide(props) {
			...
			if (records.status === "ok") return h$9("div", { className: "lp-step-body" }, h$9("div", { className: "lp-connected" }, h$9("span", {
				className: "lp-connected-icon",
				"aria-hidden": true
			}, h$9(Icon, {
				name: "check",
				size: 16
			})), h$9("div", null, h$9("div", { className: "lp-strong" }, `已连接 · ${records.indicator_count} 项指标${checkupsText(records.full_checkups)}`), h$9("div", { className: "lp-caption" }, records.latest_checkup ? `最近一次完整体检：${chineseDate(records.latest_checkup)}` : "还没有九项血检测齐的一次体检"))), h$9(CanCompute, { journey: props.journey }), h$9("div", { className: "lp-form-actions" }, recheckButton));
			return h$9("div", { className: "lp-step-body" }, records.status === "error" ? h$9("p", { className: "lp-blocker lp-blocker-bad" }, `记录读取失败：${records.error || "没有返回原因"}`) : h$9("p", { className: "lp-muted" }, "LongPi 从你自己的 Mirobody 读取体检和可穿戴数据（只读）。三步接上："), h$9("ol", { className: "lp-howto" }, h$9("li", null, "在 Mirobody 网页上传体检报告（PDF 或照片），或连接手环、手表。"), h$9("li", null, "在 Mirobody 生成个人 MCP 地址。"), h$9("li", null, "重新运行 LongPi 的安装命令，加上 ", h$9("code", null, "--mcp-url"), " 和这个地址，然后重启 DSH。")), h$9("div", { className: "lp-code" }, h$9("code", null, INSTALL_HINT), h$9("button", {
				type: "button",
				className: "lp-iconbtn",
				"aria-label": "复制安装命令",
				onClick: () => {
				...
				size: 14
			}))), h$9("div", { className: "lp-form-actions" }, recheckButton, h$9("span", { className: "lp-caption" }, records.mirobody_mounted ? "接好后点“重新检查”。" : "Mirobody 插件还没有加载，重新运行安装命令会一起装好。")));
		}
		...
		function CanCompute(props) {
			...
			const line = (label, ok, detail) => h$9("li", { className: "lp-can" }, h$9("span", {
				...
				size: 12,
				strokeWidth: ok ? 2 : 3
			})), h$9("span", { className: "lp-strong" }, label), h$9("span", { className: "lp-caption" }, detail));
			return h$9("ul", { className: "lp-cans" }, line("身体年龄", bioage.status === "ok", bioage.status === "ok" ? "现在就能算" : bioage.blocker_zh), line("心血管风险", risk.status === "ok", risk.status === "ok" ? "现在就能算" : risk.blocker_zh));
		}
		...
		function FirstResult(props) {
			...
			return h$9("div", { className: "lp-step-body" }, !props.showResults && blocked === 0 ? h$9(CanCompute, { journey: props.journey }) : null, !props.showResults ? null : h$9("div", { className: "lp-first" }, h$9("div", { className: "lp-first-cell" }, h$9("div", { className: "lp-caption" }, "身体年龄 · 模型估计"), bioage.status === "ok" ? h$9("div", null, h$9("div", { className: "lp-first-figure" }, fmt(bioage.phenoage), h$9("span", { className: "lp-bignum-unit" }, "岁")), h$9("div", { className: "lp-caption" }, [versusAge(bioage.advance), bioage.band_years != null ? `正常波动 ±${fmt(bioage.band_years)} 岁` : ""].filter(Boolean).join(" · ")), bioage.caveat_zh ? h$9("p", {
				className: "lp-caption lp-first-caveat",
				role: "note"
			}, bioage.caveat_zh) : null) : h$9("div", null, h$9("div", { className: "lp-first-wait" }, "还不能计算"), h$9("p", { className: "lp-blocker" }, bioage.blocker_zh))), h$9("div", { className: "lp-first-cell" }, h$9("div", { className: "lp-caption" }, "10 年心血管病风险 · 模型估计"), risk.status === "ok" ? h$9("div", null, h$9("div", { className: "lp-first-figure" }, riskText(risk.risk_pct), h$9("span", { className: "lp-bignum-unit" }, "%")), h$9("div", { className: "lp-caption" }, [risk.category_zh, "China-PAR"].filter(Boolean).join(" · "))) : h$9("div", null, h$9("div", { className: "lp-first-wait" }, "还不能计算"), h$9("p", { className: "lp-blocker" }, risk.blocker_zh)))), blocked > 0 ? h$9("div", { className: "lp-first-addons" }, h$9("div", { className: "lp-subhead" }, `还差 ${blocked} 项检查`, h$9("span", { className: "lp-optional" }, "下次体检加测，或现在自己量")), h$9(AddonList, {
				journey: props.journey,
				onNotice: props.onNotice,
		...
		function ProfileEditor(props) {
			...
			return h$8("form", {
				...
				noValidate: true
			}, onboarding ? null : h$8("div", { className: "lp-field" }, h$8("label", {
				className: "lp-field-label",
				htmlFor: `${props.idPrefix}-name`
			}, "称呼", h$8("span", { className: "lp-optional" }, "选填")), h$8("input", {
			...
			})), h$8("div", { className: "lp-profile-basics" }, h$8("div", { className: "lp-field lp-field-age" }, h$8("label", {
				className: "lp-field-label",
				htmlFor: `${props.idPrefix}-age`
			}, "实足年龄"), h$8("div", { className: "lp-input-unit" }, h$8("input", {
			...
			}), h$8("span", { className: "lp-unit" }, "岁"))), h$8("div", { className: "lp-field lp-field-sex" }, h$8(Segmented, {
			...
			}))), h$8("p", { className: "lp-unlock" }, h$8(Icon, {
				name: "lock",
				size: 12
			}), `解锁：${unlockOf(props.journey, "age")}`), h$8("fieldset", { className: "lp-facts" }, h$8("legend", { className: "lp-facts-legend" }, h$8("span", { className: "lp-strong" }, "心血管风险还需要这 6 项"), h$8("span", { className: "lp-caption" }, ` · 已回答 ${answeredFacts} 项，不确定就选“不确定”，不会当作“否”`)), ...facts.map((row) => h$8("div", {
				className: "lp-fact",
				key: row.key
			}, h$8("div", { className: "lp-fact-text" }, h$8("div", {
				className: "lp-fact-label",
				id: `${props.idPrefix}-${row.key}-text`
			}, row.label_zh), h$8("div", { className: "lp-unlock lp-unlock-inline" }, `解锁：${row.unlocks_zh || "心血管风险"}`, row.men_only ? female ? " · 女性的公式不用这一项，可以跳过" : " · 只用于男性的公式" : "")), h$8(Segmented, {
				...
				className: "lp-form-error",
				role: "alert"
			}, error) : null, h$8("div", { className: onboarding ? "lp-modal-actions" : "lp-form-actions" }, onboarding ? h$8(Btn, {
				variant: "outline",
				type: "button",
			...
			}, busy ? "保存中…" : onboarding ? "保存并继续" : "保存档案"), !onboarding && !dirty && props.journey?.profile.complete ? h$8("span", { className: "lp-caption" }, "已是最新") : null));
		}
		//#endregion
		...
		function Dots(props) {
			return h$7("div", { className: "lp-onb-progress" }, h$7("ol", {
				className: "lp-dots",
				"aria-hidden": true
			}, ...TITLES.map((_, index) => h$7("li", {
				key: index,
				className: `lp-dot-step ${index === props.step ? "lp-dot-now" : index < props.step ? "lp-dot-past" : ""}`
			}))), h$7("span", { className: "lp-caption" }, `第 ${props.step + 1} 步，共 ${TITLES.length} 步`));
		}
		...
		function Onboarding(props) {
			...
			return h$7(OnboardingModal, { title: TITLES[step] ?? TITLES[0] }, h$7("div", {
				className: "lp lp-onb",
				ref: content
			}, h$7(Dots, { step }), h$7("h2", {
				className: "lp-onb-title",
				tabIndex: -1
			}, TITLES[step]), step === 0 ? h$7("div", { className: "lp-onb-body" }, h$7(ConsentText), error ? h$7("p", {
				className: "lp-form-error",
				role: "alert"
			}, error) : null, h$7("div", { className: "lp-modal-actions" }, h$7(Btn, {
				variant: "outline",
				onClick: finish,
				disabled: busy
			}, "以后再说"), h$7(Btn, {
			...
			}, busy ? "记录中…" : "开始"))) : null, step === 1 ? h$7("div", { className: "lp-onb-body" }, h$7("p", { className: "lp-onb-lead" }, "只问能解锁结果的问题。每一项都可以跳过，跳过就是“不知道”，不会当作“否”。"), h$7(ProfileEditor, {
				journey,
				variant: "onboarding",
				idPrefix: "lp-onb-profile",
				onSaved: () => go(2),
				onSkip: () => go(2)
			})) : null, step === 2 ? h$7("div", { className: "lp-onb-body" }, h$7(RecordsGuide, {
				journey,
				onRecheck: () => refresh(true)
			}), h$7("div", { className: "lp-modal-actions" }, h$7(Btn, {
				variant: "outline",
				onClick: () => go(1)
			}, "上一步"), h$7(Btn, {
				"data-modal-autofocus": true,
				onClick: () => go(3)
			}, journey.records.status === "ok" ? "继续" : "先跳过"))) : null, step === 3 ? h$7("div", { className: "lp-onb-body" }, computing ? h$7("div", {
				className: "lp-onb-computing",
				"aria-busy": true
			}, h$7(Skeleton, { height: 88 }), h$7("p", { className: "lp-caption" }, "正在用你的记录计算…")) : h$7(FirstResult, {
				journey,
				onNotice: notify,
				idPrefix: "lp-onb-result",
				showResults: true
			}), h$7(FollowupOptIn), notice, h$7("p", { className: "lp-fine" }, journey.boundary_zh), h$7("div", { className: "lp-modal-actions" }, h$7(Btn, {
				variant: "outline",
				onClick: finish
			}, "完成"), h$7(Btn, {
				"data-modal-autofocus": true,
		...
		function OnboardingModal(props) {
			useInertRoot();
			return h$7(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				open: true,
				title: props.title,
				onClose: NOOP,
				headless: true,
		...
		function Spark(props) {
			...
			return h$6("div", { className: "lp-change-spark" }, h$6(LineChart, {
				points: row.points,
				unit: row.unit,
				label: row.label_zh,
				height: 56,
		...
		function ChangeRow(props) {
			const { row } = props;
			return h$6("li", { className: "lp-change" }, h$6("div", { className: "lp-change-main" }, h$6("div", { className: "lp-strong" }, row.label_zh), h$6("p", { className: "lp-change-text" }, withoutLabel(row))), h$6(Spark, { row }));
		}
		...
		function ChangesCard(props) {
			...
			return h$6("section", {
				className: "lp-card lp-changes",
				id: "lp-changes",
				"aria-labelledby": "lp-changes-title"
			}, h$6("div", {
				className: "lp-label",
				id: "lp-changes-title"
			}, "记录里的明显变化", h$6("span", { className: "lp-optional" }, `${rows.length} 项`)), ...groupsOf(rows).map((group) => h$6("div", {
				key: `${group.tone}:${group.advice}`,
				className: "lp-change-group"
			}, group.advice ? h$6("p", { className: `lp-change-advice lp-change-${group.tone}` }, h$6(Icon, {
				name: group.tone === "warn" ? "warn" : group.tone === "good" ? "check" : "info",
				size: 14
			}), h$6("span", null, group.advice)) : null, h$6("ul", { className: "lp-change-list" }, ...group.rows.map((row) => h$6(ChangeRow, {
				key: row.key,
				row
			}))))), h$6("div", { className: "lp-change-notes" }, h$6("p", { className: "lp-caption" }, "趋势图里的浅色带：以比较起点那次结果为基线的正常波动范围，落在带外就是真实变化。"), ...caveats.map((text) => h$6("p", {
				key: `caveat:${text}`,
				className: "lp-caption"
			}, text)), sources.length > 0 ? h$6("p", { className: "lp-caption lp-change-source" }, "波动数据来源：", ...sources.flatMap((source, index) => [
				index > 0 ? "；" : null,
				source.url ? h$6("a", {
					key: source.url,
					href: source.url,
			...
			])) : null, props.journey.changes_note_zh ? h$6("p", { className: "lp-fine" }, props.journey.changes_note_zh) : null));
		}
		//#endregion
		...
		*/
		function useScrollToRequested(ready) {
			const target = useScrollRequest();
			react.default.useEffect(() => {
				if (!target || !ready) return void 0;
				const started = Date.now();
		...
		function LongPiPage(props) {
			...
			if (!journey && loading) body = h$2(Loading);
				...
				body = h$2("div", { className: `lp-body ${refreshing ? "lp-refreshing" : ""}` }, early ? h$2(JourneyStepper, {
					...
					onRecheck: doRefresh,
					onNotice: notify
				}) : null, h$2(ChangesCard, { journey }), h$2(ResultsRow, {
					journey,
		...
		function Pill(props) {
			...
			return h$1("div", {
				...
				onClick: () => props.openPage?.(),
				title: summary,
				"aria-label": `LongPi 今天 ${count} 项待办：${summary}。打开健康页`
			}, h$1("span", { className: "lp-pill-mark" }, h$1(Mark, { size: 14 })), h$1("span", null, "LongPi · 今天 ", h$1("span", { className: "lp-pill-count" }, count), " 项待办")), h$1("button", {
				type: "button",
				className: "lp-pill-x",
				"aria-label": "今天不再提醒",
				onClick: () => {
