window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function Goals(props) {
			...
			}), h("div", null, h("div", { className: "lp-muted" }, "达到方案目标"), h("div", { className: "lp-tile-figure lp-good-ink-strong" }, `${fmt(pheno.goal.phenoage)} 岁`)), h("div", { className: "lp-pill lp-pill-good" }, `${fmt(pheno.goal.phenoage_delta)} 岁`)) : h("p", { className: "lp-muted" }, pheno.note_zh ?? ""), leverRows.length > 0 ? h("div", null, h("div", { className: "lp-subhead" }, "每个目标单独的贡献"), h(LeverBars, { rows: leverRows })) : sensitivityRows.length > 0 ? h("div", null, h("div", { className: "lp-subhead" }, "对你的表型年龄影响最大的指标"), h(LeverBars, { rows: sensitivityRows })) : null, pheno.goal && pheno.now?.mortality_10y_pct != null && pheno.goal.mortality_10y_pct != null ? h("div", { className: "lp-fine" }, `同一模型的 10 年死亡风险：${pheno.now.mortality_10y_pct.toFixed(1)}% → ${pheno.goal.mortality_10y_pct.toFixed(1)}%。`) : null, h("div", { className: "lp-fine" }, `${pheno.measured_on ? `按 ${pheno.measured_on} 的血检计算。` : ""}${pheno.boundary_zh ?? ""}`)) : null, risk ? h("div", { className: "lp-card lp-model" }, h("div", { className: "lp-label" }, "10 年心血管病风险 · China-PAR"), risk.status === "unavailable" ? h("div", null, h("div", { className: "lp-tile-figure lp-muted-ink" }, "待系数校验"), h("p", { className: "lp-muted" }, risk.note_zh ?? "")) : h("div", { className: "lp-model-figures" }, h("div", null, h("div", { className: "lp-muted" }, "现在"), h("div", { className: "lp-tile-figure" }, `${fmt(risk.now?.risk_pct)}%`)), risk.goal ? h(Icon, {
				name: "arrow",
				size: 20,
				className: "lp-muted-ink"
			}) : null, risk.goal ? h("div", null, h("div", { className: "lp-muted" }, "达到目标"), h("div", { className: "lp-tile-figure" }, `${fmt(risk.goal.risk_pct)}%`)) : null), h("div", { className: "lp-fine" }, risk.boundary_zh ?? "")) : null, h("div", { className: "lp-card lp-model lp-model-note" }, h("div", { className: "lp-label" }, "关于“能多活几年”"), h("p", { className: "lp-muted" }, "没有经过验证的模型能对个人给出“多活几年”。这里只给有依据的模型估计：表型年龄、同一模型的 10 年死亡风险，以及（校验通过后）中国人群的 10 年心血管病风险。"), h("p", { className: "lp-fine" }, "试验里的平均效应都不大：热量限制使衰老速度慢约 2–3%，鱼油三年约慢 3 个月。能坚持的小改变，比追逐一个数字更重要。"))));
		}
		...
		function PanelView() {
			...
			}, h("option", { value: "unknown" }, "性别未填"), h("option", { value: "female" }, "女"), h("option", { value: "male" }, "男"), h("option", { value: "other" }, "其他")), h("button", {
				type: "submit",
				className: "lp-btn",
				disabled: busy
			}, busy ? "保存中" : "保存"));
