...
`;
		...
		function DashboardView() {
			...
			const demoModules = composite ? Object.entries(composite.module_source).filter(([, s]) => s === "demo").map(([k]) => k) : [];
			return react.default.createElement("div", { className: "lp-dash" }, data.banner ? react.default.createElement("div", { className: "lp-banner" }, "演示数据 · 非个人病历 · DEMO DATA") : null, react.default.createElement("div", { className: "lp-kicker" }, `${data.brandName} · ${data.customer.display_name} · v${data.version ?? "unknown"}`), react.default.createElement("div", { className: "lp-hero" }, react.default.createElement("div", null, react.default.createElement("div", { className: "lp-north" }, react.default.createElement("div", { className: "lp-age" }, one(data.customer.composite_age)), react.default.createElement("div", { className: "lp-chrono" }, `综合生物年龄 · 实际 ${data.customer.chrono_age} 岁 · Δ ${signed(delta)}`)), pheno ? react.default.createElement("p", { className: "lp-prov" }, react.default.createElement("span", { className: "lp-prov-tag" }, "引擎计算"), ` 生物学维 = ${pheno.model}：表型年龄 ${one(pheno.phenoage)} 岁（${signed(pheno.phenoage_advance)} 年），10 年全因死亡概率 ${(pheno.mortality_10y * 100).toFixed(2)}%。`, react.default.createElement("br", null), react.default.createElement("span", { className: "lp-prov-tag lp-prov-demo" }, "演示输入"), demoModules.length ? ` 其余 ${demoModules.length} 维（${demoModules.join(" / ")}）为固定演示值，不是算出来的。` : " 五维均来自引擎。") : null, react.default.createElement("p", { className: "lp-disc" }, data.customer.disclaimer_zh)), react.default.createElement(ModuleRadar, {
				...
				className: "lp-card",
				key
			}, react.default.createElement("h3", null, mod.label_zh), react.default.createElement("div", { className: "n" }, one(mod.age)), composite && composite.module_source[key] === "engine" ? react.default.createElement("div", { className: "lp-card-src" }, "引擎") : react.default.createElement("div", { className: "lp-card-src lp-card-src-demo" }, "演示输入")))), react.default.createElement("div", { className: "lp-metrics" }, ...(data.metrics ?? []).map((m) => react.default.createElement("button", {
				key: m.code,
				type: "button",
