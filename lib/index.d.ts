...
interface Journey {
  ...
  /**
   * status partial: the record was read but some reads failed (their rows say so). summary: what the record holds,
   * for onboarding (checkup days and their span, the groups present, wearable days in the last year); null when
   * the record is not connected, or a read failed or timed out (a count would then be too small).
   */
  records: {
