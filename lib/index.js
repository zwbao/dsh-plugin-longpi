...
async function ensureBioAge(context, reference) {
	...
	if (unread.some((pair) => pair.names.length === 0)) return empty("error", `指标目录没有读全，${unread.map((pair) => pair.spec.label_zh).join("、")}可能在没有读到的部分，暂时算不出身体年龄。`);
	...
	return {
		...
		note_zh: current ? `按 ${points.length} 次同时测齐九项血检的检查回算。` : `${latest} 这次血检的身体年龄没有算出来${lastError ? `：${lastError}` : ""}。请在对话里运行身体年龄（表型年龄）方法查看原因。`,
		missing: [],
		points,
			...
			if (date) {
				...
				if (levers) {
					...
					cards.push({
						...
						note_zh: target ? `按 ${date} 的血检，达到方案目标时身体年龄 ${target.phenoage_delta != null && target.phenoage_delta <= 0 ? "年轻" : "变化"} ${fmt$2(Math.abs(target.phenoage_delta ?? 0))} 岁。` : problems.length > 0 ? `方案目标没有用于计算：${problems.join(" ")}` : "方案里还没有和九项血检对应的目标值。设定目标（如空腹血糖、超敏 CRP）后，这里会算出达到目标时的表型年龄。",
						measured_on: date,
						now: {
