...
async function cached(key, load) {
	...
	if (hit && now - hit.at < CACHE_TTL_MS$2) return hit.value;
	const value = load();
	cache.set(key, {
		at: now,
		value
	...
	for (const [name, entry] of cache) if (now - entry.at >= CACHE_TTL_MS$2) cache.delete(name);
	return value;
}
...
function dailyPoints(marker, readings) {
	...
	for (const point of sorted) {
		const factor = factorFor$1(marker, point.unit);
...
function planMatcher(context, markers) {
	...
	const resolved = resolveMarkers([.../* @__PURE__ */ new Set([...plan.items.flatMap((item) => item.markers), ...plan.goals.map((goal) => goal.marker)])], context.records.indicators, {
		z: 1.96,
		default_cva_rule_zh: "",
		markers: [...markers]
	});
...
//#endregion
export { CHANGES_NOTE_ZH, CONNECTION_FILE, CONNECTION_TEST_MS, CONNECTION_UNAVAILABLE, CONSENT_VERSION, Config, DEFAULT_FOLLOWUP, DRAFT_CATEGORIES, EMPTY_PROFILE, FOCUS, FOCUS_ZH, FOLLOWUP_MAX_PER_DAY, FOLLOWUP_TEST_TEXT, GROUP_KEYS, GROUP_ZH, HARNESS_SKILLS, PHENOAGE_SKILL, PRODUCT_VERSION, RISK_FACTS, RISK_FACT_ZH, RISK_SKILL, SELF_ALIASES, SELF_KEYS, SELF_SPEC, TOOL_NAMES, WEBHOOK_KINDS, WORKSPACE_DIR, WORKSPACE_MARKER, WORKSPACE_TITLE, acceptedPlan, addCheckIns, addDays, addSelf, adherenceFor, appendFollowupLog, apply, bootstrapWorkspace, buildBoard, buildCalendar, buildChanges, buildIndicators, buildJourney, buildJourneyFull, buildPlanBrief, buildReport, buildStats, buildTracking, cellNumber, checkupMarkerFor, clearConnection, commandExcerpt, connectionKey, connectionSource, connectionTokenProblem, connectionUrlProblem, currentPlan, daysBetween, decideFollowup, deleteSelf, desktopCommand, desktopSupported, detectIntents, domainSummary, draftPlan, effectiveConfig, effectsFor, escapeText, estimatedAge, evaluateMarker, evaluatePlan, expectedText, foldLine, foldName, followupApprovalReason, followupArmed, followupResponse, followupStateOf, followupSummary, followupTextProblem, followupTick, groupOf, guardRoute, heldUntil, homeBloodPressure, inQuiet, indicatorDetail, indicatorsFromTable, inject, invalidateIndicators, invalidateRecords, invalidateTracking, isJsonRequest, isoDay, isoWeek, isoWeekday, latestOutputs, latestSelf, loadCatalog, loadCourses, loadDoseLog, loadEvidenceLexicon, loadRecords, loadReference, loadSeries, manifestSummary, markerFor, maskMcpUrl, maskUrl, matchSkills, mentionedEntities, mergeProfile, mergeSelf, modelGoals, name, nameVariants, nextTimes, normalizePlan, normalizeProfile, normalizeUnit, organismOf, organismsAsked, parseCompact, parseFrontmatter, parseNumber, parseReadme, preGuard, profileComplete, publicFollowup, rcvBand, readCheckIns, readConnection, readFailed, readFollowup, readFollowupLog, readHistory, readPlans, readProfile, readReceipts, readResultFile, readSelf, readiness, recordOutputs, recordsSummary, rememberMedications, reportExcerpt, resolveDataDir, resolveMarkers, resolveMirobodyPlugin, resolveSkillsHome, retestDay, retestsOf, runReady, runSkill, runnableFrom, sameMeasure, saveConnection, savePlan, selfIndicators, selfSeries, sendFollowup, sendNow, sentToday, seriesOf, setConsent, setFollowupDeps, stageMeasurements, stageNow, startFollowup, suggestNext, summarizeIndicators, summarizeMedications, tableOf, testConnection, trackingGeneration, unansweredOf, unitFactor, versionCheck, webhookAnswer, webhookRequest, webhookUrlProblem, within, wrapGuardMessage, writeFollowup, writeProfile, writeStats };
