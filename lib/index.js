...
function rangeProblem(spec, value, shown, raw) {
	...
	let message = `${spec.label_zh} 读成 ${withUnit(fmt$1(value), unit)}（${shown}），不在合理范围 ${withUnit(`${fmt$1(low)}–${fmt$1(high)}`, unit)} 内。`;
	if (raw != null) {
...
function stageMeasurements(card, items) {
	...
	for (const item of items) {
		...
		if (!normalizeUnit(unit)) {
			...
			shown = factor === 1 ? `单位 ${unit}` : `原值 ${fmt$1(number)} ${unit}`;
		}
	...
	for (const spec of specs) {
		...
		const cells = [spec.key, fmt$1(values[spec.key] ?? 0)];
...
function suggestNext(summaries, context) {
	...
	for (const item of summaries) {
		if (item.adherence.level === "low" || item.adherence.level === "partial") out.push({
			kind: "adherence",
...
function chartsFor(plan, markers, series, reference, goals) {
	...
	for (const marker of markers) {
		...
		const points = (series[marker.indicator] ?? []).map((point) => ({
			date: point.date,
			value: point.value
		}));
