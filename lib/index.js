...
function nextTimes(settings, state, now, log) {
	...
	const sent = sentKeys(log);
	let checkin = null;
	if (state && state.checkin_items > 0) checkin = localIso(minutes < minutesOf(settings.checkin_time) && !sent.has(`checkin:${today}`) ? today : addDays(today, 1), settings.checkin_time);
	let retest = null;
	...
	if (settings.weekly && state?.plan_exists) {
		...
		weekly = localIso(addDays(today, offset > 0 || minutes < minutesOf(settings.weekly.time) && !sent.has(`weekly:${isoWeek(now)}`) ? offset : offset + 7), settings.weekly.time);
	}
	return {
		checkin,
		retest,
...
function changeOf(marker, points, z) {
...
function latestFor(key, indicators, biovar) {
	...
	let row;
	if (key === "waist") row = rows.find((item) => sameMeasure("waist", item));
	else {
		const marker = biovar.markers.find((item) => item.key === key);
		if (!marker) return null;
		row = rows.find((item) => item.loinc && marker.loinc.includes(item.loinc) || (marker.device_codes ?? []).includes(item.name)) ?? rows.find((item) => markerFor(biovar, item) === marker);
	}
	if (!row) return null;
	return {
...
function goalsFor(brief, chosen) {
	const out = [];
...
function followupTextProblem(text, detail) {
	if (!text) return "没有内容。";
