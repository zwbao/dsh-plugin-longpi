...
import { appendFileSync, chmodSync, existsSync, mkdirSync, readFileSync, readdirSync, realpathSync, rmSync, statSync, writeFileSync } from "node:fs";
...
const DOSE$1 = /\d+(?:\.\d+)?\s*(?:mg|mcg|µg|μg|ug|iu|g|ml|毫克|微克|国际单位|单位|克|毫升|粒|片|颗|支|滴|袋|勺)(?!\s*\/\s*(?:d?l|ml)\b)|[一二两三四五六七八九十半]+\s*(?:粒|片|颗|支|滴|袋|勺)/gi;
...
function stripDoses(value) {
	const cleaned = value.replace(DOSE$1, "").replace(/[，,]\s*(?=[，,]|$)/g, "").replace(/\s{2,}/g, " ").trim();
	return {
		text: cleaned,
...
function webhookUrlProblem(kind, url) {
...
function applyUpdate(current, update, lenient = false) {
	...
	if ("quiet" in update) {
		...
		else if (isRecord(quiet) && timeOf(quiet.start) && timeOf(quiet.end) && timeOf(quiet.start) !== timeOf(quiet.end)) next.quiet = {
...
function addSelf(dataDir, entries, opts) {
	...
	for (const value of entries.slice(0, MAX_ENTRIES)) {
		...
		const converted = round1$1(number * unit.factor);
...
function latestSelf(rows) {
	...
	for (const key of SELF_KEYS) {
		...
		if (key === "sbp" || key === "dbp") {
			...
			out[key] = {
				value: round1$1(week.reduce((sum, row) => sum + row.value, 0) / week.length),
...
function selfSeries(rows, key) {
	...
	return [...days.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([date, values]) => ({
		date,
		time: date,
		value: round1$1(values.reduce((sum, value) => sum + value, 0) / values.length),
		unit: SELF_SPEC[key].unit
	}));
}
//#endregion
...
function medChangeIn(sentence) {
	...
	if (CJK.test(lower)) return ASK_CHANGE.test(lower) || CHANGE_ASKED.test(lower) || INTENT_CHANGE.test(lower) || IMPERATIVE.test(lower) || PRESCRIBE.test(lower) && !PRESCRIBE_RECORD.test(lower);
...
function ruleLabels(input) {
	const text = String(input ?? "").normalize("NFKC").slice(0, 4e3);
...
function replyRuleCheck(reply) {
	...
	for (const line of sentences(String(reply ?? "").normalize("NFKC"))) {
		...
		if (!verdict.personal_dose && DIRECTIVE.test(lower) && (PHARMA_DOSE.test(lower) || medicine && SHARED_DOSE.test(lower))) verdict.personal_dose = true;
...
function changeOf(marker, points, z, glucoseTreated) {
...
async function buildTracking(context) {
	...
	const now = Date.now();
	const fresh = (entry) => entry.settled == null ? now - entry.started < PENDING_MAX_MS : now - entry.settled < CACHE_TTL_MS;
...
async function compute(context, plan, checkins) {
	...
	const changesRead = buildChanges(context).catch(() => ({
		changes: [],
		note_zh: CHANGES_NOTE_ZH
	}));
			...
			if (date) {
				...
				if (levers) {
					...
					cards.push({
						...
						note_zh: target ? `按 ${date} 的血检，达到方案目标时表型年龄 ${target.phenoage_delta != null && target.phenoage_delta <= 0 ? "年轻" : "变化"} ${fmt$2(Math.abs(target.phenoage_delta ?? 0))} 岁。` : "方案里还没有和九项血检对应的目标值。设定目标（如空腹血糖、超敏 CRP）后，这里会算出达到目标时的表型年龄。",
						measured_on: date,
						now: {
...
function runtimeCall(llm, route, efforts = /* @__PURE__ */ new Map()) {
	return async ({ system, user, signal }) => {
		...
		if (!efforts.has(key)) {
			let off = null;
			try {
				off = (await llm.resolveModelInfo?.(route.provider, route.model, signal))?.reasoning?.efforts?.find((effort) => /^(?:off|none|disabled)$/i.test(String(effort.id)))?.id ?? null;
			} catch {
				off = null;
			}
			efforts.set(key, off);
...
function detailFor(group, primary) {
	...
	return clipText(`${behavior}${primary.note_zh && room > 12 ? clipText(`研究备注：${primary.note_zh}`, room) : ""}${evidence}`, DETAIL_MAX);
}
...
function acceptedPlan(brief, posted, today) {
	...
	return {
		ok: true,
		plan: {
			title: typeof draft.title === "string" && draft.title.trim() ? draft.title.trim().slice(0, 60) : `改善方案（${today}）`,
			source: "board",
		...
		scoped.webServer.register({
			...
			handler: (req, res) => {
				...
				(async () => {
					...
					const posted = body.ok && body.value && typeof body.value === "object" ? body.value.draft : void 0;
...
function registerTrackingTools(ctx, config, mount) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			const records = await loadRecords(current, dataDir, mount.pluginHome);
			const today = isoDay();
			const focus = (Array.isArray(args.focus) ? args.focus : []).filter((item) => FOCUS.includes(String(item)));
...
const DOSE = new RegExp(`\\d+(?:\\.\\d+)?\\s*(?:mg|mcg|µg|μg|ug|iu|g|ml|毫克|微克|国际单位|单位|克|毫升|粒|片|颗|支|滴|袋|勺|胶囊|丸|tablets?|capsules?)(?!\\s*\\/\\s*(?:d?l|ml)\\b)|${CN_NUMBER}\\s*(?:毫克|微克|国际单位|单位|(?<!千)克|毫升|粒|片|颗|支|滴|袋|勺|胶囊|丸)|剂量`, "i");
...
function followupTextProblem(text, detail, names = []) {
	...
	if (HEALTH_VALUE.test(plain) || MARKER_NUMBER.test(plain) || /(?<![A-Za-z])\d/.test(numbers)) return "随访设置为“简要”，消息里不能有健康数值（血压、血糖、血脂、体重、百分比等）；数字只能是日期、时间或天数、项数。";
