...
const SOURCE = `${`(?:(?:${DIGITS}|${CN_NUMBER})\\s*[x×*]\\s*)?`}(?:${DIGITS}|${CN_NUMBER})\\s*(?:(?:mcg|µg|μg|ug|mg|iu|ml|g|milligrams?|micrograms?|grams?|tablets?|capsules?|pills?|drops?)(?![A-Za-z])|(?:毫克|微克|国际单位|单位|(?<!千)克|毫升|粒|片|胶囊|丸|滴|颗|支|袋|勺))(?!\\s*[/／]\\s*(?:d?l|ml|分升|升|毫升)(?![A-Za-z]))(?:\\s*[/／]\\s*(?:天|日|次|d|day)(?![A-Za-z]))?`;
...
async function loadSeries(config, names, options) {
	...
	return cached(cacheKey(config, "series", JSON.stringify([wanted, options])), async () => {
		...
		for (let start = 0; start < wanted.length; start += SERIES_CHUNK) {
			...
			const marked = table.meta.truncated || payload && typeof payload === "object" && payload.truncated === true || textOf(payload).includes("\n… cut at ");
...
async function ensureBioAge(context, reference) {
	...
	const wanted = new Map(checkups.map((date) => {
		...
		return [date, {
			measurements,
			age,
			key: inputsKey(context, date, measurements, age)
		}];
	}));
...
async function riskCard(context, reference, card, goals) {
	...
	for (const spec of measurementInputs(card)) {
		...
		if (row) found.push({
			...
			(names.some((name) => failedReads.has(name)) || names.length === 0 && context.records.catalog_truncated ? unreadLabs : missingLabs).push(spec.label_zh);
		}
	}
