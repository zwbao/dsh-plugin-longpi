...
function mergeSelf(remote, self) {
	...
	for (const row of self) {
		...
		if (remote.filter((other) => other.value && (row.loinc && other.loinc === row.loinc || devices.includes(other.name))).every((other) => (row.date ?? "") > (other.date || other.last_date || ""))) out.push(row);
	}
	return out;
}
...
async function weeklyBloodPressure(context, indicator) {
	const points = (await loadSeries(context.config, [indicator], {
		start: addDays(context.today, -30),
		end: context.today,
		resolution: "raw"
	...
	return {
		value: Math.round(week.reduce((sum, point) => sum + point.value, 0) / week.length * 10) / 10,
		unit: last.unit
	};
}
...
async function buildJourney(context) {
	const journey = journeyFrom(context, await buildTracking(context));
	lastBuilt = {
		at: Date.now(),
		journey
	};
...
function nextOf(stage, journey) {
	...
	switch (stage) {
		...
		case "plan": return step("选一项开始改善", "说出你的方案，或上传医生、长寿师给的方案。", "plan");
		case "routine": {
...
function registerTools(ctx, config, mount) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			smoker: {
				type: "boolean",
				description: "They smoke cigarettes now (China-PAR)."
			},
			diabetes: {
				type: "boolean",
				description: "They have diabetes: a diagnosis, fasting glucose at or above 7.0 mmol/L, or diabetes medicine (as they state it)."
			},
			bp_treated: {
				type: "boolean",
				description: "They took blood-pressure medicine in the last two weeks."
			},
			north: {
				type: "boolean",
				description: "They live in northern China (north of the Yangtze); false for southern China."
			},
			urban: {
				type: "boolean",
				description: "They live in a city; false for a rural area."
			},
			family_history: {
				type: "boolean",
				description: "A parent or sibling had a heart attack or stroke."
			}
		},
		output: jsonOut$1,
		timeoutMs: 1e4,
...
function onboardingOf({ journey, error }) {
	if (!journey) return {
		onboarding: null,
