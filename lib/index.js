...
const SOURCE = `${`(?:(?:${DIGITS}|${CN_NUMBER})\\s*[x×*]\\s*)?`}(?:${DIGITS}|${CN_NUMBER})\\s*(?:(?:mcg|µg|μg|ug|mg|iu|ml|g|milligrams?|micrograms?|grams?|tablets?|capsules?|pills?|drops?)(?![A-Za-z])|(?:毫克|微克|国际单位|单位|(?<!千)克|毫升|粒|片|胶囊|丸|滴))(?!\\s*[/／]\\s*(?:d?l|ml|分升|升|毫升)(?![A-Za-z]))(?:\\s*[/／]\\s*(?:天|日|次|d|day)(?![A-Za-z]))?`;
