...
运行 `dsh web` 启动 DeepSeek Harness。首次打开时先按提示填写 DeepSeek API Key；如果还没有任何工作区，LongPi 会创建一个名为「健康」的工作区。随后 LongPi 的引导分四步完成建档：
...
- **健康页**：侧栏的「健康」打开完整页面，依次是进度、记录里的明显变化、身体年龄与心血管风险、方案与打卡、档案与自测、随访提醒，以及方法库。
...
在输入框中输入 `/longpi` 可查看运行状态，包括方法库版本、Mirobody 连接情况、建档阶段和随访设置。
