...
**记录里的明显变化**：不需要方案。每个有生物变异数据的体检指标（只看带 LOINC 的检查行，手环数据和本人自测不算）用同一个标准判断：最新一次对比前一次；有三次及以上时，再对比最近六次里的第一次。变化超出参考变化值才算，两种对比都超出时报超出更多的那一个。方向是好的（按该行的 `better`）算好消息；其余的，包括没有好坏方向的指标，都建议请医生看，排在前面。按多日平均判断的指标（家庭血压）不用单次读数判断。不同医院、不同仪器之间的差异没有算进去。旅程（`changes`）、`read_personal_situation`（`record_changes`）、方案草稿的说明和导出报告显示的是同一批结果。
