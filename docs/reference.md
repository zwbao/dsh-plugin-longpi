...
| `guardScope` | Where the safety judgement asks the model. `health` (default): every message in LongPi's workspace (the one it created, or any titled 健康对话 or 健康), and in other workspaces each message that touches health, then the rest of that conversation; other messages are checked by the local rules only, with no model call and no delay. `all`: every message in every workspace, about 1.2 s each. |
